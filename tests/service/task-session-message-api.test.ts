...
describe("task session message api", () => {
  test("prefers canonical task_messages rows before legacy task_session_messages", async () => {
    ...
    const { createTaskSessionMessageApi, fakeDb } = await loadTaskSessionMessageApiModule({
      canonicalMessagesByCall: [
        ...
        {
          id: "task-session-message:session-1:assistant:cli-1",
          role: "assistant",
          status: "pending",
          clientMessageId: null,
          runtimeMessageId: "assistant:cli-1",
          seq: 9,
          textContent: null,
          textPreview: null,
          rawPayload: {},
  ...
  test("creates canonical session messages when canonical rows are absent", async () => {
    const { createTaskSessionMessageApi, fakeDb } = await loadTaskSessionMessageApiModule({
      canonicalMessagesByCall: [
        ...
        null,
        {
          id: "task-session-message:session-1:assistant:cli-2",
          role: "assistant",
          status: "pending",
          clientMessageId: null,
          runtimeMessageId: "assistant:cli-2",
          seq: 11,
          textContent: null,
          textPreview: null,
          rawPayload: {},
          errorText: null,
          createdAt: "2025-01-01T00:10:02.000Z",
          updatedAt: "2025-01-01T00:10:02.000Z",
          completedAt: null,
        },
      ],
      operationRowsByCall: [
    ...
    expect(result).toEqual({
      ...
      data: {
        ...
        assistant_message: {
          ...
          text: null,
          message_index: 11,
          created_at: "2025-01-01T00:10:02.000Z",
          completed_at: null,
        },
        operation: {
          id: "session-operation:task-1:model-request:cli-2",
    ...
    expect(upsertTaskSessionMessageRecord).toHaveBeenCalledTimes(2);
    expect(fakeDb.query.taskMessages.findFirst).toHaveBeenCalled();
  });
});
