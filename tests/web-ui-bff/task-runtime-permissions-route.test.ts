...
describe("task runtime permissions route", () => {
  test("lists runtime permissions scoped to the selected task session", async () => {
    ...
    const response = await taskRoutes.request("http://localhost/task-1/runtime-permissions?sessionId=session-1", {
  ...
  test("replies to a runtime permission request scoped to the task", async () => {
    ...
    const response = await taskRoutes.request("http://localhost/task-1/runtime-permissions/per-1/reply", {
      method: "POST",
      headers: {
