...
// ── buildExecutionPlan: single mode ────────────────────────────────

describe("buildExecutionPlan — single mode", () => {
  test("produces a single-mode plan with one candidate", () => {
    const plan = buildExecutionPlan(makeTemplate(), makeStrategy(), "general");
...
// ── buildExecutionPlan: parallel mode ──────────────────────────────

describe("buildExecutionPlan — parallel mode", () => {
  ...
  test("mode override from single to parallel", () => {
    ...
    const plan = buildExecutionPlan(template, makeStrategy(), "general", { mode: "parallel" });
    expect(plan.mode).toBe("parallel");
    expect(plan.candidates.length).toBeGreaterThanOrEqual(2);
  });
});

// ── buildExecutionPlan: sequential-chain mode ──────────────────────

describe("buildExecutionPlan — sequential-chain mode", () => {
  ...
  test("generates default step ids when not provided", () => {
    ...
    const template = makeTemplate({ mode: "sequential-chain" });
    const plan = buildExecutionPlan(template, makeStrategy(), "general", { steps });
    expect(plan.steps[0]!.id).toBe("chain-step-0");
    expect(plan.steps[1]!.id).toBe("chain-step-1");
  });
});

// ── ExecutionPlan type constraints ─────────────────────────────────

describe("ExecutionPlan type structure", () => {
