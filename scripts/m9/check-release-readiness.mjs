...
const projectRegressions = personalProjects.localImplementation?.regressionFiles ?? [];
assert(projectRegressions.length > 0, "Personal Projects regression coverage is missing");
for (const regression of projectRegressions) {
  assert(/\.(?:test\.tsx?|mjs)$/u.test(regression), `invalid regression entry: ${regression}`);
  assert(
    existsSync(path.join(root, regression)),
    `missing Personal Projects regression: ${regression}`,
  );
}
