...
const desktop = existsSync(path.join(root, "core/apps/desktop/package.json"))
  ? "core/apps/desktop"
  : "apps/desktop";
...
if (desktop.startsWith("core/")) {
  update("core/package.json", (value) => {
    value.version = version;
  });
...
if (existsSync(native)) {
  ...
  const projectAfter = projectBefore.replace(
    /MARKETING_VERSION = [^;]+;/gu,
    `MARKETING_VERSION = ${version};`,
  );
  if (checking && projectAfter !== projectBefore)
    throw new Error("PRODUCT_VERSION_DRIFT: iOS project");
...
if (existsSync(serverVersion)) {
  if (checking && readFileSync(serverVersion, "utf8").trim() !== version)
    throw new Error("PRODUCT_VERSION_DRIFT: server");
