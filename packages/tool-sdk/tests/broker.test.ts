...
describe("CapabilityBroker", () => {
  ...
  it("reuses a session grant for low-risk per-call operations only within its resource and lifetime", async () => {
    const { chat, repository, projection } = fixture();
    const execute = vi.fn(async () => ({
      summary: "applied",
      content: [],
    ...
    const broker = new CapabilityBroker(repository, [
      { operations: ["workspace_change_set_apply"], execute },
    ]);
    const operation = {
      operation: "workspace_change_set_apply" as const,
      workspaceGrantId: "11111111-1111-4111-8111-111111111111",
      workspaceChangeSetId: "22222222-2222-4222-8222-222222222222",
      idempotencyKey: "session-first-change-set",
    };
    const first = await broker.execute(projection, operation);
    if (first.status !== "permission_required") throw new Error("expected permission");
    const permission = broker.resolvePermission({
      permissionRequestId: first.permission.id,
      decision: "session",
      payloadDigest: first.permission.payloadDigest,
    });
    expect((await broker.execute(projection, operation)).status).toBe("completed");
    const next = {
      ...operation,
      workspaceChangeSetId: "33333333-3333-4333-8333-333333333333",
      idempotencyKey: "session-next-change-set",
    };
    expect((await broker.execute({ ...projection, piToolCallId: "next" }, next)).status).toBe(
      "completed",
    );
    expect(
      (
        await broker.execute(
          { ...projection, piToolCallId: "other-resource" },
          {
            ...next,
            workspaceGrantId: "44444444-4444-4444-8444-444444444444",
            idempotencyKey: "other-resource",
          },
        )
      ).status,
    ).toBe("permission_required");
    repository.revokeScope(permission.scopeId!);
    expect(
      (
        await broker.execute(
          { ...projection, piToolCallId: "revoked" },
          {
            ...next,
