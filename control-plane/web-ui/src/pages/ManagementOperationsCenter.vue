<template>
  <div style="padding: 24px">
    <a-page-header
      :title="viewModel ? `${viewModel.project.name} / 管理介入总览` : '管理介入总览'"
      sub-title="查看项目内管理决策时间线、人工覆盖历史、开放升级请求和待处理任务"
      @back="router.push(`/projects/${projectId}`)"
    />

    <ProjectSectionNav :project-id="projectId" active-key="management-operations" />
    ...
    <a-spin :spinning="loading" style="display: block">
      ...
      <template v-else-if="viewModel">
        <a-row :gutter="[16, 16]" style="margin-bottom: 16px">
          ...
          <a-col :xs="24" :md="8" :xl="4">
            <a-card size="small" title="管理决策">
              <a-statistic :value="viewModel.summary.totalManagementDecisions" />
            </a-card>
          </a-col>
          <a-col :xs="24" :md="8" :xl="4">
        ...
        <a-row :gutter="[16, 16]">
          <a-col :xs="24" :xl="12">
            <a-card size="small" title="管理决策时间线">
              <a-empty v-if="viewModel.timeline.length === 0" description="当前项目还没有管理决策记录" />
              <a-timeline v-else>
...
import {
  ...
  type ProjectManagementOperationsView,
...
const loadError = ref("");
const viewModel = ref<ProjectManagementOperationsView | null>(null);
...
async function loadData() {
  ...
  loadError.value = "";
  try {
    viewModel.value = await getProjectManagementOperationsView(projectId);
  } catch (error) {
    loadError.value =
