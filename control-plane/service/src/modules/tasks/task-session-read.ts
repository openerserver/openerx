...
import { getTaskSessionNodeId } from "../project-tree/storage";
...
export function normalizeTaskSessionLineageRecords(records: TaskSessionLineageRecord[]) {
...
export async function listTaskSessionTreeRecords(
  taskId: string,
  projectId: string,
...
export async function resolveTaskSessionRecord(
  taskId: string,
  projectId: string,
  sessionId: string,
) {
...
export async function resolveTaskSessionRecordByRuntimeSessionId(
  taskId: string,
  projectId: string,
  runtimeSessionId: string,
) {
...
export function buildConversationSessionId(taskId: string, runtimeSessionId: string) {
  return getTaskSessionNodeId(taskId, runtimeSessionId);
}
...
type TaskSessionMessagesReadSource =
  | "conversation-table"
  | "task-domain-events"
  | "conversation-table+task-domain-events";

async function loadTaskSessionMessagesPreferConversation(args: {
  taskId: string;
  runtimeSessionId: string;
}) {
  const sessionId = buildConversationSessionId(args.taskId, args.runtimeSessionId);
...
function resolveTaskSessionLineageRecords(
  preferredRecords: TaskSessionLineageRecord[],
  legacyRecords: TaskSessionLineageRecord[],
  runtimeSessionId: string,
) {
  const preferredPath = buildTaskSessionLineagePath(
    normalizeTaskSessionLineageRecords(preferredRecords),
    runtimeSessionId,
  );
  ...
  return buildTaskSessionLineagePath(
    normalizeTaskSessionLineageRecords(legacyRecords),
    runtimeSessionId,
  );
}

function mergeTaskSessionMessageReadSource(
  sources: TaskSessionMessagesReadSource[],
): TaskSessionMessagesReadSource {
...
export async function buildTaskSessionMessagesResponse(args: {
  taskId: string;
  projectId: string;
  runtimeSessionId: string;
  ...
  if (!args.includeLineage) {
    const singleSessionResult = await loadTaskSessionMessagesPreferConversation(args);
    if (singleSessionResult.data.length > 0) {
      return {
        data: singleSessionResult.data,
        meta: {
          ...
          readSource: singleSessionResult.readSource,
        },
      };
    }
  ...
  const treeRows = await listTaskSessionTreeRecords(args.taskId, args.projectId);

  const lineagePath = resolveTaskSessionLineageRecords(
    conversationRows,
    treeRows,
  ...
  const messageSets = await Promise.all(
    lineagePath.map(async (record) => ({
      record,
      ...(await loadTaskSessionMessagesPreferConversation({
        taskId: args.taskId,
  ...
  const readSource = mergeTaskSessionMessageReadSource(
...
export async function buildTaskSessionEventsResponse(args: {
  taskId: string;
  projectId: string;
  runtimeSessionId: string;
  includeLineage: boolean;
...
export async function buildTaskSessionTimelineResponse(args: {
  taskId: string;
  projectId: string;
  runtimeSessionId: string;
  includeLineage: boolean;
