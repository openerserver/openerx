...
export const runtimeUsageLedgerSteps = sqliteTable("runtime_usage_ledger_steps", {
  ...
  stepType: text("step_type", {
    enum: ["execution", "judge", "hook", "resume", "other"],
  }).notNull(),
...
export const projectTaskRelations = sqliteTable("project_task_relations", {
  ...
  targetTaskId: text("target_task_id")
    .notNull()
    .references(() => tasks.id),
  relationType: text("relation_type", { enum: ["depends-on", "blocks", "spawned-from"] }).notNull(),
...
export const roleAgents = sqliteTable("role_agents", {
  ...
  name: text("name").notNull(),
  description: text("description"),
  scope: text("scope", { enum: ["system", "project"] })
    .notNull()
    .default("system"),
...
export const roleAgentProjectOverrides = sqliteTable("role_agent_project_overrides", {
  ...
  outputSchemaId: text("output_schema_id"),
  tagsJson: text("tags_json", { mode: "json" }).$type<string[]>(),
  bindingsMode: text("bindings_mode", { enum: ["inherit", "replace"] })
    .notNull()
    .default("inherit"),
  createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
...
export const taskStageRuns = sqliteTable("task_stage_runs", {
  ...
  status: text("status", {
    ...
    .default("pending"),
  primaryRoleAgentId: text("primary_role_agent_id").notNull(),
  participantRoleAgentIdsJson: text("participant_role_agent_ids_json", { mode: "json" }).$type<
    string[]
  >(),
  startedAt: text("started_at"),
  finishedAt: text("finished_at"),
...
export const roleAggregateConclusions = sqliteTable("role_aggregate_conclusions", {
  ...
  mergedFindingsJson: text("merged_findings_json", { mode: "json" }).$type<
    Record<string, unknown>[]
  >(),
  minorityFindingsJson: text("minority_findings_json", { mode: "json" }).$type<
    Record<string, unknown>[]
  >(),
  conflictsJson: text("conflicts_json", { mode: "json" }).$type<Record<string, unknown>[]>(),
  approvalRecommendationJson: text("approval_recommendation_json", { mode: "json" }).$type<
    Record<string, unknown>
  >(),
  generatedAt: text("generated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
...
export const taskOperatingModes = sqliteTable("task_operating_modes", {
  taskId: text("task_id")
    .primaryKey()
    .references(() => tasks.id),
...
export const bossDecisions = sqliteTable("boss_decisions", {
  id: text("id").primaryKey(),
  taskId: text("task_id")
    .notNull()
    .references(() => tasks.id),
