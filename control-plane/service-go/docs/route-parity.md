...
| Crowdsourced development MVP | Task marketplace, review queue, task boundary, contributor profile, code ownership, assignment, assignment settlement, workspace branch, commit step, owner-aware CR blocking, commit runtime preview record routes, and the BFF Preview Gateway proxy entry are implemented against Go as the new Control Plane source of truth | Add richer reward ledgers and reviewer actions |
