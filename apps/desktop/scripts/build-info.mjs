...
export function createBuildInfo(root, now = new Date()) {
  ...
  let revision = "unknown", dirty = false;
  try {
  ...
  return {
    version, builtAt, revision, dirty,
...
export function getBuildInfo(root) {
  ...
  if (info.version !== expected || !/^\d{8}T\d{6}Z$/u.test(info.buildId) ||
      !/^\d{1,4}\.\d{1,2}\.\d{1,2}$/u.test(info.nativeBuildNumber) ||
      !Number.isSafeInteger(info.androidVersionCode) || info.androidVersionCode < 1 ||
      info.androidVersionCode > 2100000000 || !Number.isFinite(Date.parse(info.builtAt)) ||
      !/^(?:[a-f0-9]{40,64}|unknown)$/u.test(info.revision) || typeof info.dirty !== "boolean")
