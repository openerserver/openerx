...
test("development ignores locked native output directories, but keeps source and renderer updates", () => {
  for (const relative of [
    "out",
    "out/openerx-win32-x64/openerx.exe",
    ".native-build",
    ".native-build/openerx-credential-store",
    ...
    expect(
      isDesktopBuildArtifact(`C:/workspace/apps/desktop/${relative}`.replaceAll("/", "\\")),
    ).toBe(true);
  }
  for (const relative of [
    ...
    "src/outside.ts",
    "native/windows-desktop-helper/objects.ts",
  ]) {
