...
function targetArguments(target: MacBrowserWindow): string[] {
  return [
    target.bundleId,
    String(target.processId),
    target.title,
    String(target.bounds.x),
    String(target.bounds.y),
    String(target.bounds.width),
    String(target.bounds.height),
  ];
}
...
export class ElectronMacSystemBrowserDriver implements SystemDefaultBrowserDriver {
  ...
  async observe(
    ...
    const raw = parseMacBrowserObservation(
      await this.#runJxa(macBrowserObservationScript(), [
  ...
  async performSemantic(
    ...
    await this.#assertCurrent(binding, expectedSurface, input.observation, signal);
    const targetWindow = await this.#currentWindow(binding);
  ...
  async performNativeInput(
    ...
    const current = await this.#assertCurrent(binding, expectedSurface, input.observation, signal);
    ...
    return await this.#runAction(
      ...
      [
        ...
        ...targetArguments(targetWindow).slice(2),
        input.operation.action,
        nativeActionPayload(input),
  ...
  async closeOwnedWindow(binding: SystemBrowserBinding, signal: AbortSignal): Promise<void> {
    throwIfAborted(signal);
    const targetWindow = await this.#currentWindow(binding);
    await this.#runAppleScript(macBrowserCloseWindowScript(), targetArguments(targetWindow));
  }
  ...
  async #assertCurrent(
    ...
    if (
      ...
      currentScreenshotDigest !== expected.screenshotDigest
    ) {
      throw new Error("BROWSER_OBSERVATION_MISMATCH");
    }
    if (
      current.surface.viewport.width !== expected.viewport.width ||
  ...
  async #runAction(
    ...
    throwIfAborted(signal);
    const value = (await this.#runAppleScript(script, args)).trim();
