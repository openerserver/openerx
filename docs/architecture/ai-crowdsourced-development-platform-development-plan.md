...
在项目初期，所有参与者应根据其技能水平、过往经验和对系统架构的理解程度进行分层。

1. 初级参与者（L1）：负责低风险、边界明确的任务。
2. 合格贡献者（L2）：可参与普通业务模块。
3. 可信贡献者（L3）：可参与复杂模块或高价值任务。
4. 模块维护者（L4）：负责审核自己模块的变更。
5. 核心管理员（L5）：负责架构、合并主线、处理核心权限模块。

这种分层确保项目安全和稳定，同时让每位参与者发挥所长，形成高效协作。
