# OpenerX 运行架构图（legacy OpenCode 兼容拓扑）

> 状态说明：本文记录仓库里仍保留的 `OpenCode :4096` 兼容/回退拓扑，主要用于 legacy 联调、`control-plane/app` 排障和旧脚本复现，不代表当前默认 runtime backend。
>
> 当前默认 runtime 路径：BFF `runtime-provider` 已固定为 `pi-mono`，详见 [runtime/current-implementation-index.md](../runtime/current-implementation-index.md) 与 [runtime/pi-mono-upstream-development-plan.md](../runtime/pi-mono-upstream-development-plan.md)。

本文档描述当前仓库中仍保留的 legacy 开发态运行架构，重点标出旧 OpenCode `:4096` 兼容链路下的进程、监听端口、依赖关系，以及关键共享文件。它解释的是外部 OpenCode runtime 仍被显式启动时的工作方式，不是当前默认的 `pi-mono` backend 架构。
...
flowchart LR
    ...
    DB[("PostgreSQL")]
    OCR["OpenCode Runtime（legacy / rollback）\nopencode serve\n:4096\nopencode-fork"]
...
| Web UI BFF | `control-plane/web-ui-bff` | 4098 | `bun run src/index.ts` | 前端统一入口，JWT 校验，代理 Control Plane；当前默认执行 backend 为 `pi-mono` runtime-provider，legacy 回退时才直连 OpenCode Runtime |
| Control Plane Service | `control-plane/service` | 4097 | `bun run src/index.ts` | 认证、项目、用户、任务、审批、审计、配置主数据 |
| OpenCode Runtime（legacy / rollback） | `opencode-fork` / 本机 `opencode` 可执行文件 | 4096 | `opencode serve --hostname 127.0.0.1 --port 4096 --print-logs` | 旧 OpenCode 回退链路下的 Agent 会话执行、原始消息流、SSE 输出、模型调用、插件/MCP 调用 |
...
### 3.4 BFF 到 legacy OpenCode Runtime

- 只有在回退兼容或 legacy 联调路径下，BFF 才会直接访问 OpenCode Runtime `:4096`。
- 典型动作包括：暂停任务、恢复任务、注入指导、读取 Session 消息、订阅 SSE 事件。
- 当前默认 `pi-mono` 主路径不再依赖这个独立 HTTP runtime，而是由 BFF 通过 `runtime-provider` 托管 RPC 子进程。
- BFF 仍负责把 runtime 事件转换后再推送给前端。

这里需要锁定一条运行边界：即使在 legacy `4096` 链路里，OpenCode runtime 也只负责运行时控制、原始消息协议和实时事件输出，不再承担 task-domain execution trace 的公开读模型职责。对前端暴露的 task / project trace 视图，应以 `task_timeline_views` 为首选，并只在 projection timeline 为空或不可用时，使用由 `conversation_messages` 与 conversation domain events 聚合得到的 service timeline 补位。
...
如果你要复现这条 legacy 兼容拓扑，建议按以下顺序启动，以避免页面报错或出现 `Control plane unreachable`：
...
- 在这条 legacy 兼容链路里，Runtime 模型配置来源于 `opencode-fork/opencode.json`，而不是前端内存状态本身。
...
- legacy `OpenCode :4096` 联调或回退排障
- 新成员理解系统边界
- 解释旧兼容脚本下“页面能打开但任务列表失败”的原因
- 对齐 BFF、Control Plane、legacy Runtime 三层职责
...
flowchart TD
  ...
  OCR["4096 OpenCode Runtime（legacy）"]
...
当 legacy 兼容链路看起来“整体坏了”时，按这个顺序查最快：
