...
> 状态说明：本文是围绕旧 OpenCode adapter 链路形成的改造方案。当前默认 backend 已切到 `pi-mono` runtime-provider；文中对 OpenCode 的讨论，应理解为 legacy 适配 / 回退兼容路径以及上游协议能力分析，不代表今天的默认执行主链。
>
> 状态：Draft  
> 日期：2026-03-21  
...
当前仓库中仍保留的 legacy OpenCode adapter 路径里，BFF 调用 OpenCode Runtime 时只传递了精简参数（prompt 文本 + 模型 + agent 名称），OpenCode 在内部通过插件链完成 system prompt 注入、工具定义收集、意图分类等大量加工。BFF 对 LLM 最终接收的输入缺乏可观测性和控制力。

**目标**：将 system prompt 构造和工具白名单控制上移到 BFF，使 legacy OpenCode 适配链路退化为纯执行引擎（Agentic Loop + 工具执行 + SSE 广播），BFF 成为 prompt 编排的单一控制面。
...
└──────────────────────┬──────────────────────────────────────┘
                       ...
                       ▼
┌─────────────────────────────────────────────────────────────┐
│ Legacy OpenCode Runtime / upstream adapter（执行面 — "How to do"） │
│                                                             │
