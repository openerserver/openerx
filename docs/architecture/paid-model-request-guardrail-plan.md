...
>
> runtime 状态更新（2026-04-15）：OpenCode 已弃用，不再作为当前执行内核。本文中凡把 OpenCode runtime message/part ledger 或 opencode 内部 breaker 写成现行路径的段落，都应按历史方案理解。当前真实设计以 BFF + `runtime-provider`（默认 `pi-mono`）为执行主链，并把底层执行事实同步到 `runtime_usage_ledgers` / `runtime_usage_ledger_steps`。
...
- [docs/approval-standards-management-plan.md](../workflow/approval-standards-management-plan.md)
- [docs/multi-agent-hook-architecture.md](multi-agent-hook-architecture.md)
- [docs/operations/dashboard-provider-token-stats-plan.md](../operations/dashboard-provider-token-stats-plan.md)
...
其中并行 candidate 的成本随候选数近似线性增长，[docs/multi-agent-hook-architecture.md](multi-agent-hook-architecture.md) 已明确指出这一风险，但当前缺少强制执行的上限机制。
...
但根据 [docs/architecture-overview.md](architecture-overview.md) 的判断，这些能力尚未与运行时内核形成强约束闭环。
...
#### C. 历史 OpenCode runtime ledger：旧底层 usage 原始记录

本文成稿时，OpenCode runtime 仍是执行内核，因此当时确实存在一层比 `agent_runs` 更细的底层 usage 记录：

- runtime 会在 step finish 阶段根据 provider 返回的 usage 计算 token 与 cost
- 这些数据会写入 assistant message / part 元数据
- BFF 曾把这层数据当作回填或排障来源，而不是控制面的正式治理账本

但这部分现在只应被视为历史背景，不再代表当前设计：OpenCode 已弃用，session / message / part usage 不再是现行治理口径。

#### D. 当前设计的真值分工

当前实现里，三层真值应这样理解：

- `agentRunId` / 兼容 run 视图：运营与任务视图的兼容标识层，不再等同于独立物理 `agent_runs` 主表
- `cost_records`：控制面成本事实层，用于预算、审批、结算、审计
- `runtime_usage_ledgers` / `runtime_usage_ledger_steps`：当前已落地的底层执行事实层，由 BFF 在 runtime-provider 主链上同步，用于回答“实际发起了几次模型请求、每一步消耗多少 token、哪一个 hook / judge 放大了请求”

也就是说，runtime usage ledger 这个概念仍然代表当前设计，但它的实现已经不是 OpenCode runtime message/part ledger，而是控制面正式表 + BFF sync 链路。
...
- 执行前预估
  ...
  - 不能只看兼容 run 级统计
- 执行后回放
  - 必须能够下钻到 `runtime_usage_ledgers` / `runtime_usage_ledger_steps`
  - 展示本次执行的底层请求次数、token 分项和放大来源

当前差距不是“有没有 ledger 表”，而是“项目治理页和首页是否已经把这层事实完整消费出来”。

#### F. 当前迁移状态与后续方向

当前迁移状态应解读为：

1. `runtime_usage_ledgers` / `runtime_usage_ledger_steps` 已进入控制面 schema，并已有 sync / query 接口。
2. BFF 已在 paid execution 运行链路里同步 cost record 与 runtime usage ledger。
3. 当前剩余缺口主要是 baseline 生成、项目治理页事实回放、首页治理下钻，以及与审批 / 预算总览的进一步打通。

只有把这些消费层补齐，管理员在项目管理页看到的“预计请求次数 / token / 成本”，才会和执行后审计结果保持可解释的一致性。
...
### 10.1.6 历史 opencode 内部实时激增告警与临时熔断（已废弃）

> 历史口径说明：本节描述的是 OpenCode 仍作为执行内核时的 breaker 设想。由于 OpenCode 已弃用，这套“把实时熔断内建在 opencode 本地 runtime 与 permission 通道里”的设计不再代表当前实现。
>
> 当前实现边界：运行中 breaker 已落在 BFF 的 paid execution runtime / realtime aggregator / agent-control 主链中，负责累计 runtime usage、触发 breaker、终止剩余 candidate / hook，并同步审计与 runtime usage ledger。当前没有采用 opencode 本地库表 + `PermissionNext.ask/reply` 作为现行审批恢复路径。

除了执行前预估，本方案还应补一层“执行中实时保护”。

目标不是等整次 suite 跑完后再审计，而是当某一小段时间窗口内请求量突然放大时，
在该历史设想里，由 opencode runtime 立刻进入临时熔断，暂停后续模型请求，等待管理员审批。

这项能力放在 opencode 内部是当时可行的历史设想，原因是旧源码曾具备三类基础设施：
...
历史设想中，核心判断逻辑放在 opencode 内部，优先接在 `SessionProcessor` 的 step 生命周期上。
...
也就是说，在这套历史设想里，实时熔断判断必须内建在 opencode；这已经不是当前实现。当前主链里的实时 breaker 落在 BFF paid execution runtime / realtime aggregator，审批与治理展示仍回到 OpenerX 控制面。

### 10.1.7 历史 opencode 代码落点清单（已废弃）

> 当前代码落点应优先参考以下现行实现，而不是下文的 opencode 文件清单：
>
> - [control-plane/web-ui-bff/src/lib/paid-execution-runtime.ts](../../control-plane/web-ui-bff/src/lib/paid-execution-runtime.ts)
...
> - [control-plane/web-ui-bff/src/modules/agent-control/run-persistence.ts](../../control-plane/web-ui-bff/src/modules/agent-control/run-persistence.ts)
> - [control-plane/service/src/modules/projects/routes.ts](../../control-plane/service/src/modules/projects/routes.ts)
...
1. 已有统一的 paid model preflight / guard 核心逻辑。BFF 已实现模型分级、风险预估、准入判定、自动降级建议与 guard state 结构，核心位于 [control-plane/web-ui-bff/src/lib/paid-execution-guard.ts](../../control-plane/web-ui-bff/src/lib/paid-execution-guard.ts)。

1. 已有项目级 fund snapshot / reserve / consume / refund / ledger 能力。Service 侧已提供项目额度钱包与流水接口，见 [control-plane/service/src/modules/projects/routes.ts](../../control-plane/service/src/modules/projects/routes.ts) 与 [control-plane/service/src/db/schema.pg.ts](../../control-plane/service/src/db/schema.pg.ts)。BFF 侧已代理 fund 读写与流水查询，并在执行链路中把项目可用余额作为当前准入边界，见 [control-plane/web-ui-bff/src/modules/projects/routes.ts](../../control-plane/web-ui-bff/src/modules/projects/routes.ts)、[control-plane/web-ui-bff/src/lib/project-fund.ts](../../control-plane/web-ui-bff/src/lib/project-fund.ts) 与 [control-plane/web-ui-bff/src/modules/tasks/routes.ts](../../control-plane/web-ui-bff/src/modules/tasks/routes.ts)。

1. 任务执行与继续执行已经接入执行前阻断。`execute` 与 `continue` 在创建 runtime session 或发送后续 prompt 之前，都会先跑 preflight；命中 deny / require-approval 时会直接返回，不再先放流量再记账，见 [control-plane/web-ui-bff/src/modules/tasks/routes.ts](../../control-plane/web-ui-bff/src/modules/tasks/routes.ts)。

1. 已有付费执行安全覆盖层。当命中付费模型 guard 时，当前实现会对高风险编排做安全收敛：parallel collapse、judge disable、post-hook disable，并将状态写回 task strategy，而不是原样继承持久化策略。这部分逻辑位于 [control-plane/web-ui-bff/src/modules/tasks/routes.ts](../../control-plane/web-ui-bff/src/modules/tasks/routes.ts)。

1. 运行中 breaker 已覆盖主执行与关键放大路径。当前已经把 runtime 使用量累计、超限后熔断、剩余 candidate 终止、hook 停止等能力接入主执行链路。相关代码位于 [control-plane/web-ui-bff/src/lib/paid-execution-runtime.ts](../../control-plane/web-ui-bff/src/lib/paid-execution-runtime.ts)、[control-plane/web-ui-bff/src/modules/realtime/sse-aggregator.ts](../../control-plane/web-ui-bff/src/modules/realtime/sse-aggregator.ts)、[control-plane/web-ui-bff/src/modules/agent-control/routes.ts](../../control-plane/web-ui-bff/src/modules/agent-control/routes.ts)。

1. pre-execution / pre-resume / judge / post-hook 的使用量记账已接入。当前不是只统计主 agent run；pre-execution hook、pre-resume hook、judge、post-execution / on-failure hook 的 token 使用与审计也已经进入治理链路。

1. cost records 与 paid execution audit 已形成基础闭环。Service 侧已提供 `cost_records` 写入口，见 [control-plane/service/src/modules/cost/routes.ts](../../control-plane/service/src/modules/cost/routes.ts)。BFF 侧会在 runtime 使用量落库时同步写 `cost_records` 与 `paid_execution` 审计，见 [control-plane/web-ui-bff/src/modules/agent-control/run-persistence.ts](../../control-plane/web-ui-bff/src/modules/agent-control/run-persistence.ts)。

1. 设置页与 Dashboard 已暴露当前治理入口。设置页已提供“测试专用模型”配置，并强制测试模型只从允许列表中选择，见 [control-plane/web-ui/src/pages/Settings.vue](../../control-plane/web-ui/src/pages/Settings.vue)。Dashboard 已提供治理总览区，当前展示 block、breaker、Top 风险任务以及运行态摘要，不再暴露活跃租约数，见 [control-plane/web-ui/src/pages/Dashboard.vue](../../control-plane/web-ui/src/pages/Dashboard.vue) 与 [control-plane/service/src/modules/dashboard/routes.ts](../../control-plane/service/src/modules/dashboard/routes.ts)。

1. 测试链路已补齐针对当前实现的回归覆盖。已有 preflight、breaker、judge usage、项目预检、设置页、任务页等单测 / 集成回归。近期已验证通过根目录 `bun run typecheck`、`bun run test:bff:paid-execution-regression`，以及 hooks / completion sync / identity binding / workflow evaluation 四组真实执行验证。

### 14.2 当前已经落地，但与原方案描述不完全一致的边界

以下是当前实现边界，必须明确说明，避免把“目标状态”误当成“现状行为”：

1. 当前测试默认策略是“强制低成本测试模型优先”，不是“所有真实执行一律必须带 paid gate + lease”。当前测试模型配置通过 [control-plane/web-ui-bff/src/modules/config/routes.ts](../../control-plane/web-ui-bff/src/modules/config/routes.ts) 的 `/api/config/models/test-policy` 暴露；设置页允许的测试模型只有 `github-copilot:gpt-5-mini` 与 `github-copilot:gpt-4o`；现实行为是默认受控模型优先把真实测试导向低成本路径，而不是把每一次真实执行都当成高成本 paid run 处理。

1. `ALLOW_PAID_MODEL_EXECUTION=1` 已退出当前真实执行测试合同。当前真实执行集成只要求 `RUN_EXECUTION_INTEGRATION=1`，并统一从设置页测试模型策略选择受控模型，见 [tests/README.md](../../tests/README.md)。fund 不足、downgrade 建议与 guard / breaker 路径通过强 mock 回归覆盖，而不是靠额外 env gate 手动切换。

1. `github-copilot:gpt-4o` 当前只是“允许在测试策略里选择”，并不等于“无条件跳过治理判定”。它是否触发 paid execution 降级建议、额度预留或 breaker 路径，取决于当前 billing / guard 解析与项目 fund，而不是额外 env 开关，见 [control-plane/web-ui-bff/src/lib/paid-execution-guard.ts](../../control-plane/web-ui-bff/src/lib/paid-execution-guard.ts)。

1. 项目页当前展示的是 guard 预估，不是 runtime ledger 真值回放。现在项目页能看到请求区间、token 区间、成本区间、risk drivers 和额度相关上下文；但这些数据仍属于 BFF 的启发式 preflight 估算，不是基于 `runtime_usage_ledgers` / `runtime_usage_ledger_steps` 的下钻式事实回放。

1. 当前 runtime breaker 已经能在执行中止损，但组织级预算与审批体系还没有完全接上。也就是说，现状已经具备“运行前阻断 + 运行中熔断 + 审计留痕”的基础闭环；但“组织级配额、用户级预算、正式审批工作流、首页治理总览”还没有完整落地。

### 14.3 尚未落地或仅部分落地的规划项

以下内容仍应视为后续工作，不应误认为已经上线：

1. `model_execution_policies`、`guard_events` 等完整治理表还未按方案落地。当前真正已落表的是历史 `paid_execution_leases` 残留；模型策略主体仍以代码侧规则和启发式映射为主，而不是数据库驱动的可运营策略系统。

1. runtime usage ledger 基础账本已落地，但消费链路尚未完整。`runtime_usage_ledgers`、`runtime_usage_ledger_steps` 已进入控制面 schema，并已有 sync / query 接口；当前未完整落地的是 `runtime_usage_baselines`、项目治理页下钻式事实回放，以及首页对 ledger 事实层的完整消费。

1. 组织级 / 项目级 / 用户级预算治理尚未完整落地。当前 guard 已能基于模型档位与单次上界做限制，但项目日预算、月预算、用户级测试预算、组织级高成本配额等规划项还未形成完整执行体系。

1. 审批标准体系尚未真正闭环到执行入口。当前已有历史 lease 上下文与风险提示残留，但文中设想的 `audit_only / require_approval / block_only` 多档审批动作、管理员审批恢复等能力，还没有全部接入。

1. 首页 / 治理页已具备第一版可见性，但仍不是完整运营工作台。当前 Dashboard 已有 `blockedCount`、`breakerCount`、`topRiskTaskCount`、`runningTaskCount`、`activeSessionCount` 等摘要与风险列表，但组织级预算、审批恢复、事实回放导出等能力仍未完整落地。

1. 真实执行测试的三层拆分只部分落地。当前已经有默认回归与低成本真实执行回归的明显分层趋势，但“付费执行验证”作为单独的人控测试层，还没有完全独立为一套稳定脚本和操作规约。
...
## 15. 下一步任务拆解（优先补 runtime usage ledger 消费链路与首页治理视图）

本节把第 14.3 节中的缺口进一步拆成可执行任务，优先级按以下顺序排列：

1. 先补 runtime usage ledger 的消费链路与 baseline
2. 再补首页治理视图
3. 最后再补组织级预算、审批恢复与完整治理总览

这样安排的原因很直接：

- 没有把 runtime usage ledger 真正消费到页面和 baseline，首页治理页仍只能展示启发式预估，无法提供执行后事实回放
- 没有首页治理视图，已存在的 guard / breaker / 预检能力只能算“内核能力”，还不能形成管理员可用的治理操作面

### 15.1 Epic A：完善 runtime usage ledger 消费链路

目标：把已经落地的 runtime usage ledger 事实层真正接入 baseline、项目治理页和首页总览，让“执行后到底打了几次模型、每一步消耗了多少 token、是谁放大了请求”可被管理员直接消费。

#### A1. Service 校正 runtime usage ledger 头表与 bridge 对齐

任务目标：继续完善 `runtime_usage_ledgers` 头表与当前 `task_runs` / `task_run_nodes` / project tree bridge 的对齐，而不是重新新增一张表。
...
1. 头表 schema、migration metadata 与 service route normalization 保持一致。
2. 同一 `runtime_session_id` 不会重复写入多条头记录。
3. 头表能稳定关联现有 `cost_records`、task/run bridge 与项目治理查询。

#### A2. Service 完善 runtime usage ledger step 明细表与查询

任务目标：继续完善 `runtime_usage_ledger_steps`，记录 execution / judge / hook 等逐步调用事实，并支撑下钻查询与回放。
...
#### A3. BFF 巩固 runtime usage ledger 写入协调器

任务目标：在现有 BFF ledger sync 基础上继续收口所有 paid execution 放大路径，避免遗漏 judge / hook / resume / candidate 的逐步事实同步。
...
该 epic 需要与 [docs/operations/dashboard-provider-token-stats-plan.md](../operations/dashboard-provider-token-stats-plan.md) 以及 [runtime/raw-audit-trace-plan.md](../runtime/raw-audit-trace-plan.md) 对齐。
...
- 第一阶段：`audit_events` + `runtime_usage_ledgers` + `task_snapshots` + `task_timeline_views`
- 第二阶段：叠加 `cost_records` 与更细粒度的 runtime ledger step 明细
...
1. 首页可以直接看出当前系统是否处于“guard 高频拦截”“breaker 频繁触发”“Top 风险任务堆积”“运行态异常增加”等状态。
...
2. 首页可以直接看到最近窗口内的 block / breaker / top risk tasks / runtime state 摘要。
