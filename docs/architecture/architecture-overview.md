...
>
> 当前正式实现已移除旧版图模型兼容链路。涉及历史图镜像或旧同步机制的描述，应以 [docs/dag-node-execution-plan-v2.md](../workflow/dag-node-execution-plan-v2.md) 的最新结论为准。
>
> runtime 状态说明：当前默认 backend 已切到 `pi-mono` runtime-provider。OpenCode runtime 只保留历史边界说明与回退兼容语境；当前实现请优先参考 [runtime/current-implementation-index.md](../runtime/current-implementation-index.md)，历史资料请看 [archive/runtime/README.md](../archive/runtime/README.md)。
...
- [runtime 当前实现索引](../runtime/current-implementation-index.md)
- [OpenCode 历史边界说明](../archive/runtime/historical-opencode-focus-boundary.md)
- [legacy OpenCode 兼容拓扑图](runtime-process-architecture.md)
...
- 对 runtime backend 的适配与控制（当前默认 `pi-mono`，OpenCode 仅回退兼容）

从职责边界看，系统由“控制平面服务 + 前端聚合层 + Web UI + 外部运行时”四部分组成。

关于 runtime 能力边界，需要单独强调两层语义：当前默认实现和验收路径见 [runtime/current-implementation-index.md](../runtime/current-implementation-index.md)；旧 OpenCode 的历史关注边界与协议背景见 [archive/runtime/README.md](../archive/runtime/README.md)。不要再把历史 OpenCode 能力边界直接写成当前默认 runtime 主路径。
...
- 主要职责：
  ...
  - 校验前端 JWT
  - 将任务、审批等查询代理到控制平面服务
  - 通过 runtime-provider 对接默认 `pi-mono` backend，执行暂停、恢复、注入指导、终止等控制动作
  - 将 runtime 事件转为系统统一事件，并通过 WebSocket 推送给前端
...
### 3.4 当前默认 Runtime Backend（pi-mono）

当前默认执行 backend 不是独立的 OpenCode HTTP runtime，而是由 BFF 通过 `runtime-provider` 托管的 `pi-mono` RPC 子进程。OpenCode runtime 现在只保留为历史资料和回退兼容路径，不再是当前主链默认实现。

- 默认形态：BFF 内部托管 `pi-mono` RPC 子进程，无固定对外端口
- 主要职责：
  - 管理 agent/session 生命周期
  - 提供 prompt、guidance、resume、terminate 与消息读取能力
  - 输出 runtime event/message 流，并桥接审批与治理扩展

当前系统与默认 runtime backend 的关系仍然是“控制平面治理 + 运行时执行”分离，只是执行器已经从独立 OpenCode service 收敛为 BFF provider 托管的 `pi-mono` child process。

在 execution trace 这一条链路上，这种分离需要落到更具体的实现约束：

1. 默认 runtime backend 提供会话控制、原始消息和事件流。
...
模型设计覆盖治理元数据、Workflow 阶段推进、执行记录与审批治理几个方向。当前执行主方案见 [dag-node-execution-plan-v2.md](../workflow/dag-node-execution-plan-v2.md)。
...
1. 当前默认 runtime backend 输出 runtime 事件；在 legacy OpenCode 回退链路下，这一层表现为 SSE 事件。
...
- `/api` 与 `/ws` 统一转发到 BFF
- BFF 再回源控制平面服务，并托管默认 `pi-mono` runtime backend；OpenCode 只保留回退兼容路径
...
详细的运行时穿透能力实现，见 [oh-my-openagent 实现说明](../research/oh-my-openagent-implementation.md)。
