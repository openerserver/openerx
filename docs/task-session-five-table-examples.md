/**
 * Tool dispatch. Every security decision from plan 鎼�2 is enforced HERE,
 ...
 *      exempted 閳ワ拷 it threads the ungranted state to the renderer so the
 *      user can grant TCC perms from inside the approval dialog.
 *   3. Tool-specific gates (see dispatch table) 閳ワ拷 ANY exception in a gate
 *      returns a tool error, executor never called.
 *   4. Executor call.
 *
 * For input actions (click/type/key/scroll/drag/move_mouse) the tool-specific
 * gates are, in order:
 *   a. `prepareForAction` 閳ワ拷 hide every non-allowlisted app, then defocus us
 ...
 *   b. Frontmost gate 閳ワ拷 branched by actionKind:
 *        mouse:    frontmost 閳拷 allowlist 閳拷 {hostBundleId, Finder} 閳拷 pass.
 *                  hostBundleId passes because the executor's
 *                  `withClickThrough` bracket makes us click-through.
 *        keyboard: frontmost 閳拷 allowlist 閳拷 {Finder} 閳拷 pass.
 *                  hostBundleId 閳拷 ERROR (safety net 閳ワ拷 defocus should have
 *                  moved us off; if it didn't, typing would go into our
 *                  own chat box).
 *      After step (a) this gate fires RARELY 閳ワ拷 only when something popped
 ...
 * free text 閳ワ拷 error messages may contain file paths / app content (PII).
 */
...
/**
 * Telemetry payload piggybacked on the result 閳ワ拷 populated by handlers,
 ...
 * field 閳ワ拷 the host wrapper strips it before returning to the SDK.
 */
export type CuCallToolResult = CallToolResult & {
  screenshot?: ScreenshotResult;
  /** Piggybacked telemetry 閳ワ拷 stripped by the host wrapper before SDK return. */
...
// ---------------------------------------------------------------------------
// Arg validation 閳ワ拷 lightweight, no zod (mirrors chrome-mcp's cast-and-check)
// ---------------------------------------------------------------------------
 ...
 *     fresh per tool call 閳ワ拷 never cached across calls 閳ワ拷
 ...
 *     screenshot-px 閳拷 logical-pt ratio is `displayWidth / screenshotWidth`,
 *     NOT `1/scaleFactor`. Uses the display geometry stashed at CAPTURE time
 *     (`lastScreenshot.displayWidth`), not fresh 閳ワ拷 so the transform matches
 *     what the model actually saw even if the user changed display settings
 *     since. (Chrome's ScreenshotContext pattern 閳ワ拷 CDPService.ts:1486-1493.)
 */
function scaleCoord(
  rawX: number,
  rawY: number,
  ...
  if (lastScreenshot) {
    ...
    // ComputerTool.swift:70-80 閳ワ拷 two independent convergent impls.
  ...
  // Degenerate 閳ワ拷 fall back to the old /sf behavior and warn.
  logger.warn(
...
/**
 * Convert model-space coordinates to the 0閳ワ拷100 percentage that
 ...
 * denominator is just `lastScreenshot.width` 閳ワ拷 the model's raw pixel coord is
...
function coordToPercentageForPixelCompare(
  ...
  if (mode === "normalized_0_100") {
    // Unchanged 閳ワ拷 already a percentage.
...
/**
 * Tier needed to perform a given action class. `undefined` 閳拷 `"full"`.
 *
 *   - `"mouse_position"` 閳ワ拷 mouse_move only. Passes at any tier including
 *     `"read"`. Pure cursor positioning, no app interaction. Still runs
 *     prepareForAction (hide non-allowed apps).
 *   - `"mouse"` 閳ワ拷 plain left click, double/triple, scroll, drag-from.
 *     Requires tier `"click"` or `"full"`.
 *   - `"mouse_full"` 閳ワ拷 right/middle click, any click with modifiers,
 *     drag-drop (the `to` endpoint of left_click_drag). Requires tier
 *     `"full"`. Right-click 閳拷 context menu Paste, modifier chords 閳拷
 *     keystrokes before click, drag-drop 閳拷 text insertion at the drop
 ...
 *   - `"keyboard"` 閳ワ拷 type, key, hold_key. Requires tier `"full"`.
 */
...
// around the gate (osascript, System Events, cliclick via Bash) 閳ワ拷 this
// closes that door explicitly. Leading space so it concatenates cleanly.
const TIER_ANTI_SUBVERSION =
  " Do not attempt to work around this restriction 閳ワ拷 never use AppleScript, " +
  "System Events, shell commands, or any other method to send clicks or " +
  "keystrokes to this app.";

// ---------------------------------------------------------------------------
// Clipboard guard 閳ワ拷 stash+clear while a click-tier app is frontmost
// ---------------------------------------------------------------------------
//
...
// clipboard holds `rm -rf /` 閳ワ拷 from the user, from a prior full-tier paste,
// OR from the agent's own write_clipboard call (which doesn't route through
// runInputActionGates) 閳ワ拷 a left_click on that button injects it.
//
...
// re-clear is the load-bearing part 閳ワ拷 a stash-on-transition-only design
...
// cuHiddenDuringTurn unhide) 閳ワ拷 reads `session.cuClipboardStash` directly and
...
async function syncClipboardStash(
  ...
  if (!frontmostIsClickTier) {
    // Restore + clear. Idempotent 閳ワ拷 if nothing is stashed, no-op.
    if (current === undefined) return;
    try {
      await adapter.executor.writeClipboard(current);
      // Clear only after a successful write 閳ワ拷 a transient pasteboard
      // failure must not irrecoverably drop the stash.
      overrides.onClipboardStashChanged?.(undefined);
    } catch {
      // Best effort 閳ワ拷 stash held, next non-click action retries.
    }
    return;
  }
  ...
  if (current === undefined) {
    try {
      ...
      // readClipboard failed 閳ワ拷 use empty sentinel so we don't retry the stash
  ...
  // Paste button 閳ワ拷 the next action's clear clobbers the agent's write
 ...
 * Any throw inside 閳拷 caught by handleToolCall's outer try 閳拷 tool error. */
async function runInputActionGates(
  ...
  // Step A+B 閳ワ拷 hide non-allowlisted apps + defocus us. Sub-gated. After this
  ...
  // ALL grant tiers stay visible 閳ワ拷 visibility is the baseline (tier "read").
  ...
  // After handleToolCall's tier backfill, every grant has a concrete tier 閳ワ拷
  ...
  // Clipboard guard. Per-action, not per-tool-call 閳ワ拷 runs for every sub-action
  ...
  // phase don't trigger a sync 閳ワ拷 only input actions do.
  if (subGates.clipboardGuard) {
    await syncClipboardStash(adapter, overrides, frontmostTier === "click");
  }

  if (!frontmost) {
    // No frontmost app (rare 閳ワ拷 login window?). Let it through; the click
  ...
  if (frontmostTier !== undefined) {
    ...
    // guidance to the actual tier 閳ワ拷 at "read", suggesting left_click or Bash
    // is wrong (nothing is allowed; use Chrome MCP). At "click", the
    // mouse_full/keyboard-specific messages apply.
    if (frontmostTier === "read") {
      // tier "read" is not category-unique (browser AND trading map to it) 閳ワ拷
      ...
      const isBrowser =
        ...
        "browser";
      return errorResult(
        `"${frontmost.displayName}" is granted at tier "read" 閳ワ拷 ` +
    ...
    if (actionKind === "keyboard") {
      return errorResult(
        `"${frontmost.displayName}" is granted at tier "click" 閳ワ拷 ` +
  ...
  if (frontmost.bundleId === hostBundleId) {
    if (actionKind !== "keyboard") {
      // mouse and mouse_full are both click events 閳ワ拷 click-through works.
      // We're click-through (executor's withClickThrough). Pass.
      return null;
    }
    // Keyboard safety net 閳ワ拷 defocus (prepareForAction step B) should have
  ...
  // Non-allowlisted, non-us, non-Finder. RARE after the hide loop 閳ワ拷 means
  ...
  return errorResult(
    `"${frontmost.displayName}" is not in the allowed applications and is ` +
      `currently in front. Take a new screenshot 閳ワ拷 it may have appeared ` +
      `since your last one.`,
    "app_not_granted",
  );
}
 ...
 * overlapping it 閳ワ拷 `runInputActionGates` passes (frontmost is fine), but the
 ...
 * falls through 閳ワ拷 the frontmost check in `runInputActionGates` already ran.
 */
async function runHitTestGate(
  ...
  // Finder (desktop, file dialogs) is always clickable 閳ワ拷 same exemption as
  ...
  if (!tierByBundleId.has(target.bundleId)) {
    ...
    // is the "something popped up" edge case 閳ワ拷 a new window appeared between
  ...
  // the hit-test target's tier 閳ワ拷 if target is click-tier, stash+clear
  ...
  // mouse_full 閳拷 click; the only remaining fall-through is tier "read".
  if (actionKind === "mouse_full" && targetTier === "click") {
    return errorResult(
      `Click at these coordinates would land on "${target.displayName}", ` +
        `which is granted at tier "click" 閳ワ拷 right-click, middle-click, and ` +
...
/**
 * 鎼�6 item 9 閳ワ拷 screenshot retry on implausibly-small buffer. Battle-tested
...
// ---------------------------------------------------------------------------
// Grapheme iteration 閳ワ拷 鎼�6 item 7, ported from the Vercept acquisition
// ---------------------------------------------------------------------------

const INTER_GRAPHEME_SLEEP_MS = 8; // 鎼�6 item 4 閳ワ拷 125 Hz USB polling
 ...
 * reset on every lock acquire (handleToolCall 閳拷 acquireCuLock branch) so
 ...
 *  drop) 閳ワ拷 hit-test at "mouse", not "mouse_full". */
let mouseMoved = false;

/** Clears the cross-call drag flags. Called from Gate-3 on lock-acquire and
 *  from `bindSessionContext` in mcpServer.ts 閳ワ拷 a fresh lock holder must not
 ...
 *  handleClick. No-op when not held 閳ワ拷 callers don't need to check. */
 ...
 * show an approval dialog for access it can't use) but defer ACQUIRE 閳ワ拷 the
...
function resolveRequestedApps(
  ...
  for (const app of installed) {
    ...
    // candidates in the dialog) is plan-documented but deferred 閳ワ拷 the
  ...
  return requestedNames.map((requested): ResolvedAppRequest => {
    ...
    // installed 閳ワ拷 the reverse-DNS string won't match any display-name substring).
...
async function handleRequestAccess(
  ...
  if (overrides.getTeachModeActive?.()) {
    return errorResult(
      "Cannot request additional permissions during teach mode 閳ワ拷 the permission dialog would be hidden. End teach mode (finish the tour or let the turn complete), then call request_access, then start a new tour.",
      "teach_mode_conflict",
    );
  }
  ...
  // then clicks "Ask again" 閳ワ拷 both buttons resolve with deny by design
  ...
  if (tccState) {
    ...
    // dialog was up. The `tccState` arg is a pre-dialog snapshot 閳ワ拷 reading
    ...
    if (recheck.granted) {
      return errorResult(
        "macOS Accessibility and Screen Recording are now both granted. " +
          "Call request_access again immediately 閳ワ拷 the next call will show " +
  ...
  // Filter tieredApps to what was actually granted 閳ワ拷 if the user unchecked
  ...
  // Best-effort 閳ワ拷 grants are already persisted by wrappedPermissionHandler;
  ...
  return okJson(
    {
      granted: allGranted,
      denied: dialogDenied,
      // Policy blocklist 閳ワ拷 precedes userDenied in precedence and response
      ...
      // User-configured auto-deny 閳ワ拷 stripped before the dialog; this is the
    ...
    {
      // dialogGranted only 閳ワ拷 skipDialogGrants are idempotent re-grants of
 ...
 *
 * Does the full app-name閳墳nstalledApp resolution, assigns each a tier
 * (browser閳拷"read", terminal/IDE閳拷"click", else "full" 閳ワ拷 see deniedApps.ts),
...
interface TieredApp {
  bundleId: string;
  displayName: string;
  /** Never `"full"` 閳ワ拷 only restricted tiers are collected. */
  tier: "read" | "click";
}
...
interface AccessRequestParts {
  ...
  /** Resolved apps with `proposedTier !== "full"` 閳ワ拷 for the guidance text.
   ...
   *  etc. 閳ワ拷 `deniedApps.isPolicyDenied`). Precedence over userDenied. */
...
async function buildAccessRequest(
  ...
  // before userDenied 閳ワ拷 checks bundle ID AND display name (covers
  ...
  // User-configured auto-deny (Settings 閳拷 Desktop app 閳拷 Computer Use).
  // Stripped BEFORE
  // tier assignment 閳ワ拷 these never reach the dialog regardless of category.
  ...
  // it's not installed) are omitted 閳ワ拷 they'll end up in the `denied` list
  ...
  // awaits are fine 閳ワ拷 the Swift module is cached (listInstalledApps above
  ...
  const skipDialogGrants: AppGrant[] = skipDialog
    ...
    .map((r) => {
      ...
      // synthesizing a new one 閳ワ拷 keeps Settings-page "Granted 3m ago" honest.
...
function buildTierGuidanceMessage(tiered: TieredApp[]): string {
  // tier "read" is not category-unique 閳ワ拷 split so browsers get the CiC hint
  ...
  if (readBrowsers.length > 0) {
    ...
    parts.push(
      `${names} ${readBrowsers.length === 1 ? "is a browser" : "are browsers"} 閳ワ拷 ` +
  ...
  if (clickTier.length > 0) {
    ...
    parts.push(
      `${names} ${clickTier.length === 1 ? "has" : "have"} terminal or IDE ` +
        `capabilities 閳ワ拷 granted at tier "click" (visible + plain left-click ` +
  ...
  // Same anti-subversion clause the gate errors carry 閳ワ拷 said upfront so the
...
function buildUserDeniedGuidance(
  ...
  return (
    ...
    `(Settings 閳拷 Desktop app (General) 閳拷 Computer Use 閳拷 Denied apps). ` +
    `Requests for ` +
    ...
    `deny list in Settings 閳ワ拷 you cannot request this through the tool.`
  );
}

/**
 * Guidance for policy-denied apps (baked-in blocklist, not user-editable).
 * Unlike userDenied, there is no escape hatch 閳ワ拷 the agent is told to find
 * another approach.
 */
...
/**
 * Telemetry helper 閳ワ拷 counts by category. Field names (`denied_*`) are kept
 ...
 * success. No grant-flag checkboxes (clipboard/systemKeys) in teach mode 閳ワ拷
 ...
 * must explicitly consent to (main window hides) 閳ワ拷 idempotent app grants
...
async function handleRequestTeachAccess(
  ...
  // Same as handleRequestAccess above 閳ワ拷 the dialog renders in the hidden
  ...
  // TCC-ungranted branch 閳ワ拷 identical to handleRequestAccess's. The renderer
  ...
  if (tccState) {
    ...
    // Same re-check as handleRequestAccess 閳ワ拷 user may have granted while the
    // dialog was up, and the pre-dialog snapshot would mislead the model.
    const recheck = await adapter.ensureOsPermissions();
    if (recheck.granted) {
      return errorResult(
        "macOS Accessibility and Screen Recording are now both granted. " +
          "Call request_teach_access again immediately 閳ワ拷 the next call will " +
  ...
  // 閳ワ拷 skip the dialog entirely. Without this, onTeachPermissionRequest fires
  // with apps:[] and the user sees an empty approval dialog where Allow and
  // Deny produce the same result (granted=[] 閳拷 teachModeActive stays false).
  ...
  // skipDialogGrants are pre-existing idempotent app grants 閳ワ拷 they don't
  ...
  // app was previously granted (worst case: needDialog=[] 閳拷 Allow and
  ...
  return okJson(
    ...
    {
      // response.granted only 閳ワ拷 skipDialogGrants are idempotent re-grants.
...
// ---------------------------------------------------------------------------
// teach_step + teach_batch 閳ワ拷 shared step primitives
// ---------------------------------------------------------------------------
 ...
 * batch 閳ワ拷 same invariant as computer_batch's "coordinates refer to the
 ...
 * prepareForAction ONCE per step (the user clicked Next 閳ワ拷 they consented to
 ...
 *
 * Empty `actions` is valid 閳ワ拷 "read this, click Next to continue" steps.
...
async function executeTeachStep(
  ...
  // onPermissionRequest 閳ワ拷 host stores the resolver, overlay IPC fires it.
  ...
  for (const [i, act] of step.actions.entries()) {
    // Same abort check as handleComputerBatch 閳ワ拷 Exit calls stopSession so
    ...
    // Drop mid-step screenshot piggyback 閳ワ拷 same invariant as computer_batch.
 ...
 * runs its own prepareForAction 閳ワ拷 that's correct: actions may have
 * opened something outside the allowlist. The .screenshot piggyback
 * flows through to serverDef.ts's stash 閳拷 lastScreenshot updates 閳拷
...
async function appendTeachScreenshot(
  ...
  if (shotResult.isError) {
    // Hide+screenshot failed (rare 閳ワ拷 e.g. SCContentFilter error). Don't
  ...
  return {
    content: [
      ...
      // image]. Spread all 閳ワ拷 both notes are useful context and the model
...
async function handleTeachStep(
  ...
  // ok. No screenshot for empty actions 閳ワ拷 screen didn't change, model's
 ...
 * steps 閳拷 one model閳墣PI round trip instead of N. Each step still blocks for
 ...
 * user has already clicked through steps 1閳ワ拷4.
 *
 * Anchors for every step scale against the pre-call `lastScreenshot` 閳ワ拷 same
 ...
 *
 * Result shape:
 *   {exited: true, stepsCompleted: N}                   閳ワ拷 user clicked Exit
 *   {stepsCompleted, stepFailed, executed, failed, 閳ヮ洠   閳ワ拷 action error at step N
 *   {stepsCompleted, results: [...]} + screenshot       閳ワ拷 all steps ran
 */
async function handleTeachBatch(
  ...
  // Validate upfront 閳ワ拷 fail fast before showing any tooltip.
...
async function buildHiddenNote(
  ...
  return (
    ...
    `${one ? "it" : "one of them"}, that's why you don't see it 閳ワ拷 call ` +
...
function uniqueDisplayLabels(
  ...
  // NSScreen.screens iteration order 閳ワ拷 same label always maps to same
  // physical display across buildMonitorNote 閳拷 switch_display round-trip,
 ...
 * screenshots get no text 閳ワ拷 avoids noise.
 */
async function buildMonitorNote(
  ...
  // hot-unplug) must not tank the screenshot 閳ワ拷 this note is optional context.
  let displays;
  try {
  ...
  // pre-multimon) 閳ワ拷 treat same as undefined.
...
async function handleScreenshot(
  ...
  // 鎼�2 閳ワ拷 empty allowlist 閳拷 tool error, no screenshot.
  ...
  // Atomic resolve閳姫repare閳妽apture (one Swift call, no scheduler gap).
  // Off 閳拷 fall through to separate-calls path below.
  if (subGates.autoTargetDisplay) {
    // Model's explicit switch_display pin overrides everything 閳ワ拷 Swift's
    ...
    const result = await adapter.executor.resolvePrepareCapture({
      ...
      autoResolve,
      // Keep the hideBeforeAction sub-gate independently rollable 閳ワ拷
    ...
    // so no retry here 閳ワ拷 just a warning when the result is implausibly
    ...
    if (
      ...
      adapter.logger.warn(
        `[computer-use] resolvePrepareCapture result implausibly small (${decodedByteLength(result.base64)} bytes decoded) 閳ワ拷 possible transient display state`,
      );
    }
    ...
    // autoResolve (not just appSetChanged) 閳ワ拷 when pinned, we didn't
    ...
    // Cherry-pick 閳ワ拷 don't spread `result` (would leak resolver fields into lastScreenshot).
  ...
  // 閳ワ拷 if a non-allowlisted app is on top, SCContentFilter would composite it
  ...
  if (subGates.hideBeforeAction) {
    ...
    // expected-noise hide. NOT a delta against a cumulative set 閳ワ拷 that was
    ...
    // delta to zero. The double-click 閳拷 Preview opens 閳拷 re-hide 閳拷 silent
    // loop never breaks.
    //
    // With this check: every re-hide fires. If the model loops "click 閳拷 file
    // opens in Preview 閳拷 screenshot 閳拷 Preview hidden", it gets told EVERY
    // time. Eventually it'll request_access for Preview (or give up).
    //
    // False positive: user alt-tabs mid-turn 閳拷 Safari re-hidden 閳拷 reported.
    // Rare, and "Safari appeared" is at worst mild noise 閳ワ拷 far better than
...
async function handleZoom(
  ...
  // region: [x0, y0, x1, y1] in IMAGE-PX of lastScreenshot 閳ワ拷 same space the
  ...
  if (x1 > last.width || y1 > last.height) {
    return errorResult(
      `region exceeds screenshot bounds (${last.width}鑴�${last.height})`,
      "bad_args",
    );
  }

  // image-px 閳拷 logical-pt. Same ratio as scaleCoord (:198-199) 閳ワ拷
  ...
  // Return the image. NO `.screenshot` piggyback 閳ワ拷 this is the invariant.
  return {
...
async function handleClickVariant(
  ...
  // the OS button 閳ワ拷 but without this, the JS flag stays true and all
  ...
  // left_click(coordinate=[x,y], text="shift") 閳ワ拷 hold modifiers
  ...
  if (args.text !== undefined) {
    ...
    // via native.key(m, "press") 閳ワ拷 a non-modifier like "q" in text="cmd+q"
    // gets pressed while Cmd is held 閳拷 Cmd+Q fires before the click.
    if (
  ...
  // 鎼�6 item P 閳ワ拷 pixel-validation staleness check. Sub-gated.
  ...
  if (subGates.pixelValidation) {
    ...
    const validation = await validateClickTarget(
      ...
      async () => {
        ...
        try {
          ...
          // selection 閳ワ拷 pixel-compare is against the model's last image.
    ...
    if (!validation.valid && validation.warning) {
      // Warning result 閳ワ拷 model told to re-screenshot.
...
async function handleType(
  ...
  // 鎼�6 item 3 閳ワ拷 clipboard-paste fast path for multi-line. Sub-gated AND
  ...
  // 鎼�6 item 7 閳ワ拷 grapheme-cluster iteration. Prevents ZWJ emoji 閳拷 閿燂拷.
  // 鎼�6 item 4 閳ワ拷 8ms between graphemes (125 Hz USB polling). Battle-tested:
  ...
  //      after stripping the newline 閳ワ拷 virtualKey 0 is the 'a' key, so a
...
async function handleKey(
  ...
  // 鎼�2 閳ワ拷 blocklist check BEFORE gates. A blocked combo with an ungranted
  // app frontmost should return the blocklist error, not the frontmost
  // error 閳ワ拷 the model's fix is to request the flag, not change focus.
  if (
...
async function handleScroll(
  ...
  // up 閳拷 dy = -amount; down 閳拷 dy = +amount; left 閳拷 dx = -amount; right 閳拷 dx = +amount.
  ...
  // a leftMouseDragged event (enigo reads NSEvent.pressedMouseButtons) 閳ワ拷
...
async function handleDrag(
  ...
  // across the drag and desyncs the flag from OS state 閳ワ拷 same mechanism as
  ...
  // (required). `start_coordinate` is OPTIONAL 閳ワ拷 when omitted, drag from
  ...
  // else: rawFrom stays undefined 閳拷 executor drags from current cursor.
  ...
  // omitted the drag begins at the cursor 閳ワ拷 same bypass as mouse_move 閳拷
  ...
  // right-click閳墾aste. `from` stays "mouse" 閳ワ拷 picking up is a read.
...
async function handleMoveMouse(
  ...
  // the window under the cursor 閳ワ拷 that's interaction, not positioning.
  ...
  // hit-test the cursor to close the mouse_move閳姡eft_mouse_down decomposition).
  ...
  if (mouseButtonHeld) {
    // "mouse_full" 閳ワ拷 same as left_click_drag's to-endpoint. Dragging onto a
...
async function handleOpenApplication(
  ...
  // Resolve display-name 閳拷 bundle ID. Same logic as request_access.
  ...
  if (looksLikeBundleId(app) && allowed.has(app)) {
    ...
    // Try display name 閳拷 bundle ID, but ONLY against the allowlist itself.
  ...
  // open_application works at any tier 閳ワ拷 bringing an app forward is exactly
  ...
  // clicking on dock icons. Single-monitor 閳拷 no hint. listDisplays failure is
  // non-fatal 閳ワ拷 the hint is advisory.
    ...
    if (displayCount >= 2) {
      return okText(
        `Opened "${app}". If it isn't visible in the next screenshot, it may ` +
          `have opened on a different monitor 閳ワ拷 use switch_display to check.`,
      );
    }
  }
...
async function handleSwitchDisplay(
  ...
  // Resolve label 閳拷 displayId fresh. Same source buildMonitorNote reads,
...
async function handleReadClipboard(
  ...
  // read_clipboard doesn't route through runInputActionGates 閳ワ拷 sync here so
  ...
  // clipboardGuard may have stashed+cleared 閳ワ拷 read the actual (possibly
...
async function handleWriteClipboard(
  ...
  if (subGates.clipboardGuard) {
    ...
    if (frontmost && frontmostTier === "click") {
      return errorResult(
        `"${frontmost.displayName}" is a tier-"click" app and currently ` +
          ...
          `would clear the clipboard anyway 閳ワ拷 a UI Paste button in this ` +
    ...
    // write_clipboard doesn't route through runInputActionGates 閳ワ拷 sync here
 ...
 * No frontmost gate 閳ワ拷 no input, nothing to protect. Kill-switch + TCC
 ...
 * coordinateSpace annotation 閳ワ拷 the model handles both shapes.
 *
 * When lastScreenshot is present: inverse of scaleCoord 閳ワ拷 logical points 閳拷
 * image-pixels via `imageX = logicalX 鑴� (screenshotWidth / displayWidth)`.
 * Uses capture-time dims so the returned coords match what the model would
 * read off that screenshot.
 *
 * No frontmost gate 閳ワ拷 read-only, no input.
 */
 ...
 * as wait. Keyboard action 閳拷 frontmost gate applies; same systemKeyCombos
...
async function handleHoldKey(
  ...
  // Blocklist check BEFORE gates 閳ワ拷 same reasoning as handleKey. Holding
...
async function handleLeftMouseDown(
  ...
  // overlapping a tier-"full" frontmost app 閳ワ拷 bypassing runHitTestGate's
...
async function handleLeftMouseUp(
  ...
  // Any gate rejection here must release the button FIRST 閳ワ拷 otherwise the
  ...
  // vector) 閳ワ拷 hit-test at "mouse_full" same as left_click_drag's `to`. When
  // NO move happened, this is a click-release 閳ワ拷 same semantics as the atomic
...
/**
 * Executes `actions: [{action, 閳ヮ洠, 閳ヮ泝`
 * sequentially in ONE model閳墣PI round trip 閳ワ拷 the dominant latency cost
 ...
 *   - Frontmost gate: checked PER ACTION. State can change mid-batch 閳ワ拷 a
 ...
 * false, pixelValidation: false}` to each inner dispatch 閳ワ拷 the handlers'
 ...
 *
 * Mid-batch screenshots are allowed (for inspection) but NEVER piggyback 閳ワ拷
...
async function handleComputerBatch(
  ...
  // design). Frontmost still checked 閳ワ拷 runInputActionGates does it
  // unconditionally.
  ...
  const results: BatchActionResult[] = [];
  for (const [i, act] of actions.entries()) {
    // Overlay Stop 閳拷 host's stopSession 閳拷 lifecycleState leaves "running"
    // synchronously before query.interrupt(). The SDK abort tears down the
    // host's await but not this loop 閳ワ拷 without this check the remaining
    ...
    if (inner.isError) {
      ...
      // reflects the actual failure 閳ワ拷 without this, batch-internal errors
 ...
 * AFTER kill-switch + TCC gates have passed. Never sees request_access 閳ワ拷 it's
...
export async function handleToolCall(
  ...
  //     added the app to Settings 閳拷 Desktop app 閳拷 Computer Use 閳拷 Denied apps)
  //     must not survive. Without
  //     this, a stale grant bypasses the auto-deny. Stripped silently 閳ワ拷 the
  ...
  //     `tier: undefined`, which `tierSatisfies` treats as `"full"` 閳ワ拷 wrong
  ...
  // 閳光偓閳光偓閳光偓 Gate 1: kill switch 閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓
  ...
  // 閳光偓閳光偓閳光偓 Gate 2: TCC 閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓
  // Accessibility + Screen Recording on macOS. Pure check 閳ワ拷 no dialog,
  ...
  // 閳光偓閳光偓閳光偓 Gate 3: global CU lock 閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓
  // At most one session uses CU at a time. Every tool including
  // request_access hits the CHECK 閳ワ拷 even showing the approval dialog while
  ...
  //
  // request_teach_access is NOT in this set 閳ワ拷 approving teach mode HIDES
  ...
  // singleton. When undefined (tests/future hosts), no gate 閳ワ拷 absence of
  // the mechanism 閳拷 locked out.
  ...
  if (lock) {
    ...
    if (lock.holder === undefined && !deferAcquire) {
      // Acquire. Emits cuLockChanged 閳拷 overlay shows. Idempotent 閳ワ拷 if
      ...
      // Fresh lock holder 閳拷 any prior session's mouseButtonHeld is stale
    ...
    // lock.isSelf 閳拷 already held by us, proceed.
    // lock.holder === undefined && deferAcquire 閳拷
    //   checked but not acquired 閳ワ拷 proceed, first action will acquire.
  }

  // Sub-gates read FRESH every call so a GrowthBook flip takes effect
  // mid-session (plan 鎼�3).
  const subGates = adapter.getSubGates();

  // Clipboard guard runs per-action inside runInputActionGates + inline in
  // handleReadClipboard/handleWriteClipboard. NOT here 閳ワ拷 per-tool-call sync
  ...
  // 閳光偓閳光偓閳光偓 Fail-closed dispatch 閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓
  // ANY exception below 閳拷 tool error, executor never left in a half-called
  // state. Explicit inversion of the prior `catch 閳拷 return true` fail-open.
  try {
    ...
    // the gated tools 閳ワ拷 the gates run before the executor in every handler.
    // For ungated tools, the executor may have been mid-call; that's fine 閳ワ拷
