/**
 * Tool dispatch. Every security decision from plan 搂2 is enforced HERE,
 ...
 *      exempted 鈥� it threads the ungranted state to the renderer so the
 *      user can grant TCC perms from inside the approval dialog.
 *   3. Tool-specific gates (see dispatch table) 鈥� ANY exception in a gate
 *      returns a tool error, executor never called.
 *   4. Executor call.
 *
 * For input actions (click/type/key/scroll/drag/move_mouse) the tool-specific
 * gates are, in order:
 *   a. `prepareForAction` 鈥� hide every non-allowlisted app, then defocus us
 ...
 *   b. Frontmost gate 鈥� branched by actionKind:
 *        mouse:    frontmost 鈭� allowlist 鈭� {hostBundleId, Finder} 鈫� pass.
 *                  hostBundleId passes because the executor's
 *                  `withClickThrough` bracket makes us click-through.
 *        keyboard: frontmost 鈭� allowlist 鈭� {Finder} 鈫� pass.
 *                  hostBundleId 鈫� ERROR (safety net 鈥� defocus should have
 *                  moved us off; if it didn't, typing would go into our
 *                  own chat box).
 *      After step (a) this gate fires RARELY 鈥� only when something popped
 ...
 * free text 鈥� error messages may contain file paths / app content (PII).
 */
...
/**
 * Telemetry payload piggybacked on the result 鈥� populated by handlers,
 ...
 * field 鈥� the host wrapper strips it before returning to the SDK.
 */
export type CuCallToolResult = CallToolResult & {
  screenshot?: ScreenshotResult;
  /** Piggybacked telemetry 鈥� stripped by the host wrapper before SDK return. */
...
// ---------------------------------------------------------------------------
// Arg validation 鈥� lightweight, no zod (mirrors chrome-mcp's cast-and-check)
// ---------------------------------------------------------------------------
 ...
 *     fresh per tool call 鈥� never cached across calls 鈥�
 ...
 *     screenshot-px 鈫� logical-pt ratio is `displayWidth / screenshotWidth`,
 *     NOT `1/scaleFactor`. Uses the display geometry stashed at CAPTURE time
 *     (`lastScreenshot.displayWidth`), not fresh 鈥� so the transform matches
 *     what the model actually saw even if the user changed display settings
 *     since. (Chrome's ScreenshotContext pattern 鈥� CDPService.ts:1486-1493.)
 */
function scaleCoord(
  rawX: number,
  rawY: number,
  ...
  if (lastScreenshot) {
    ...
    // ComputerTool.swift:70-80 鈥� two independent convergent impls.
  ...
  // Degenerate 鈥� fall back to the old /sf behavior and warn.
  logger.warn(
...
/**
 * Convert model-space coordinates to the 0鈥�100 percentage that
 ...
 * denominator is just `lastScreenshot.width` 鈥� the model's raw pixel coord is
...
function coordToPercentageForPixelCompare(
  ...
  if (mode === "normalized_0_100") {
    // Unchanged 鈥� already a percentage.
...
/**
 * Tier needed to perform a given action class. `undefined` 鈫� `"full"`.
 *
 *   - `"mouse_position"` 鈥� mouse_move only. Passes at any tier including
 *     `"read"`. Pure cursor positioning, no app interaction. Still runs
 *     prepareForAction (hide non-allowed apps).
 *   - `"mouse"` 鈥� plain left click, double/triple, scroll, drag-from.
 *     Requires tier `"click"` or `"full"`.
 *   - `"mouse_full"` 鈥� right/middle click, any click with modifiers,
 *     drag-drop (the `to` endpoint of left_click_drag). Requires tier
 *     `"full"`. Right-click 鈫� context menu Paste, modifier chords 鈫�
 *     keystrokes before click, drag-drop 鈫� text insertion at the drop
 ...
 *   - `"keyboard"` 鈥� type, key, hold_key. Requires tier `"full"`.
 */
...
// around the gate (osascript, System Events, cliclick via Bash) 鈥� this
// closes that door explicitly. Leading space so it concatenates cleanly.
const TIER_ANTI_SUBVERSION =
  " Do not attempt to work around this restriction 鈥� never use AppleScript, " +
  "System Events, shell commands, or any other method to send clicks or " +
  "keystrokes to this app.";

// ---------------------------------------------------------------------------
// Clipboard guard 鈥� stash+clear while a click-tier app is frontmost
// ---------------------------------------------------------------------------
//
...
// clipboard holds `rm -rf /` 鈥� from the user, from a prior full-tier paste,
// OR from the agent's own write_clipboard call (which doesn't route through
// runInputActionGates) 鈥� a left_click on that button injects it.
//
...
// re-clear is the load-bearing part 鈥� a stash-on-transition-only design
...
// cuHiddenDuringTurn unhide) 鈥� reads `session.cuClipboardStash` directly and
...
async function syncClipboardStash(
  ...
  if (!frontmostIsClickTier) {
    // Restore + clear. Idempotent 鈥� if nothing is stashed, no-op.
    if (current === undefined) return;
    try {
      await adapter.executor.writeClipboard(current);
      // Clear only after a successful write 鈥� a transient pasteboard
      // failure must not irrecoverably drop the stash.
      overrides.onClipboardStashChanged?.(undefined);
    } catch {
      // Best effort 鈥� stash held, next non-click action retries.
    }
    return;
  }
  ...
  if (current === undefined) {
    try {
      ...
      // readClipboard failed 鈥� use empty sentinel so we don't retry the stash
  ...
  // Paste button 鈥� the next action's clear clobbers the agent's write
 ...
 * Any throw inside 鈫� caught by handleToolCall's outer try 鈫� tool error. */
async function runInputActionGates(
  ...
  // Step A+B 鈥� hide non-allowlisted apps + defocus us. Sub-gated. After this
  ...
  // ALL grant tiers stay visible 鈥� visibility is the baseline (tier "read").
  ...
  // After handleToolCall's tier backfill, every grant has a concrete tier 鈥�
  ...
  // Clipboard guard. Per-action, not per-tool-call 鈥� runs for every sub-action
  ...
  // phase don't trigger a sync 鈥� only input actions do.
  if (subGates.clipboardGuard) {
    await syncClipboardStash(adapter, overrides, frontmostTier === "click");
  }

  if (!frontmost) {
    // No frontmost app (rare 鈥� login window?). Let it through; the click
  ...
  if (frontmostTier !== undefined) {
    ...
    // guidance to the actual tier 鈥� at "read", suggesting left_click or Bash
    // is wrong (nothing is allowed; use Chrome MCP). At "click", the
    // mouse_full/keyboard-specific messages apply.
    if (frontmostTier === "read") {
      // tier "read" is not category-unique (browser AND trading map to it) 鈥�
      ...
      const isBrowser =
        ...
        "browser";
      return errorResult(
        `"${frontmost.displayName}" is granted at tier "read" 鈥� ` +
    ...
    if (actionKind === "keyboard") {
      return errorResult(
        `"${frontmost.displayName}" is granted at tier "click" 鈥� ` +
  ...
  if (frontmost.bundleId === hostBundleId) {
    if (actionKind !== "keyboard") {
      // mouse and mouse_full are both click events 鈥� click-through works.
      // We're click-through (executor's withClickThrough). Pass.
      return null;
    }
    // Keyboard safety net 鈥� defocus (prepareForAction step B) should have
  ...
  // Non-allowlisted, non-us, non-Finder. RARE after the hide loop 鈥� means
  ...
  return errorResult(
    `"${frontmost.displayName}" is not in the allowed applications and is ` +
      `currently in front. Take a new screenshot 鈥� it may have appeared ` +
      `since your last one.`,
    "app_not_granted",
  );
}
 ...
 * overlapping it 鈥� `runInputActionGates` passes (frontmost is fine), but the
 ...
 * falls through 鈥� the frontmost check in `runInputActionGates` already ran.
 */
async function runHitTestGate(
  ...
  // Finder (desktop, file dialogs) is always clickable 鈥� same exemption as
  ...
  if (!tierByBundleId.has(target.bundleId)) {
    ...
    // is the "something popped up" edge case 鈥� a new window appeared between
  ...
  // the hit-test target's tier 鈥� if target is click-tier, stash+clear
  ...
  // mouse_full 鈭� click; the only remaining fall-through is tier "read".
  if (actionKind === "mouse_full" && targetTier === "click") {
    return errorResult(
      `Click at these coordinates would land on "${target.displayName}", ` +
        `which is granted at tier "click" 鈥� right-click, middle-click, and ` +
...
/**
 * 搂6 item 9 鈥� screenshot retry on implausibly-small buffer. Battle-tested
...
// ---------------------------------------------------------------------------
// Grapheme iteration 鈥� 搂6 item 7, ported from the Vercept acquisition
// ---------------------------------------------------------------------------

const INTER_GRAPHEME_SLEEP_MS = 8; // 搂6 item 4 鈥� 125 Hz USB polling
 ...
 * reset on every lock acquire (handleToolCall 鈫� acquireCuLock branch) so
 ...
 *  drop) 鈥� hit-test at "mouse", not "mouse_full". */
let mouseMoved = false;

/** Clears the cross-call drag flags. Called from Gate-3 on lock-acquire and
 *  from `bindSessionContext` in mcpServer.ts 鈥� a fresh lock holder must not
 ...
 *  handleClick. No-op when not held 鈥� callers don't need to check. */
 ...
 * show an approval dialog for access it can't use) but defer ACQUIRE 鈥� the
...
function resolveRequestedApps(
  ...
  for (const app of installed) {
    ...
    // candidates in the dialog) is plan-documented but deferred 鈥� the
  ...
  return requestedNames.map((requested): ResolvedAppRequest => {
    ...
    // installed 鈥� the reverse-DNS string won't match any display-name substring).
...
async function handleRequestAccess(
  ...
  if (overrides.getTeachModeActive?.()) {
    return errorResult(
      "Cannot request additional permissions during teach mode 鈥� the permission dialog would be hidden. End teach mode (finish the tour or let the turn complete), then call request_access, then start a new tour.",
      "teach_mode_conflict",
    );
  }
  ...
  // then clicks "Ask again" 鈥� both buttons resolve with deny by design
  ...
  if (tccState) {
    ...
    // dialog was up. The `tccState` arg is a pre-dialog snapshot 鈥� reading
    ...
    if (recheck.granted) {
      return errorResult(
        "macOS Accessibility and Screen Recording are now both granted. " +
          "Call request_access again immediately 鈥� the next call will show " +
  ...
  // Filter tieredApps to what was actually granted 鈥� if the user unchecked
  ...
  // Best-effort 鈥� grants are already persisted by wrappedPermissionHandler;
  ...
  return okJson(
    {
      granted: allGranted,
      denied: dialogDenied,
      // Policy blocklist 鈥� precedes userDenied in precedence and response
      ...
      // User-configured auto-deny 鈥� stripped before the dialog; this is the
    ...
    {
      // dialogGranted only 鈥� skipDialogGrants are idempotent re-grants of
 ...
 *
 * Does the full app-name鈫扞nstalledApp resolution, assigns each a tier
 * (browser鈫�"read", terminal/IDE鈫�"click", else "full" 鈥� see deniedApps.ts),
...
interface TieredApp {
  bundleId: string;
  displayName: string;
  /** Never `"full"` 鈥� only restricted tiers are collected. */
  tier: "read" | "click";
}
...
interface AccessRequestParts {
  ...
  /** Resolved apps with `proposedTier !== "full"` 鈥� for the guidance text.
   ...
   *  etc. 鈥� `deniedApps.isPolicyDenied`). Precedence over userDenied. */
...
async function buildAccessRequest(
  ...
  // before userDenied 鈥� checks bundle ID AND display name (covers
  ...
  // User-configured auto-deny (Settings 鈫� Desktop app 鈫� Computer Use).
  // Stripped BEFORE
  // tier assignment 鈥� these never reach the dialog regardless of category.
  ...
  // it's not installed) are omitted 鈥� they'll end up in the `denied` list
  ...
  // awaits are fine 鈥� the Swift module is cached (listInstalledApps above
  ...
  const skipDialogGrants: AppGrant[] = skipDialog
    ...
    .map((r) => {
      ...
      // synthesizing a new one 鈥� keeps Settings-page "Granted 3m ago" honest.
...
function buildTierGuidanceMessage(tiered: TieredApp[]): string {
  // tier "read" is not category-unique 鈥� split so browsers get the CiC hint
  ...
  if (readBrowsers.length > 0) {
    ...
    parts.push(
      `${names} ${readBrowsers.length === 1 ? "is a browser" : "are browsers"} 鈥� ` +
  ...
  if (clickTier.length > 0) {
    ...
    parts.push(
      `${names} ${clickTier.length === 1 ? "has" : "have"} terminal or IDE ` +
        `capabilities 鈥� granted at tier "click" (visible + plain left-click ` +
  ...
  // Same anti-subversion clause the gate errors carry 鈥� said upfront so the
...
function buildUserDeniedGuidance(
  ...
  return (
    ...
    `(Settings 鈫� Desktop app (General) 鈫� Computer Use 鈫� Denied apps). ` +
    `Requests for ` +
    ...
    `deny list in Settings 鈥� you cannot request this through the tool.`
  );
}

/**
 * Guidance for policy-denied apps (baked-in blocklist, not user-editable).
 * Unlike userDenied, there is no escape hatch 鈥� the agent is told to find
 * another approach.
 */
...
/**
 * Telemetry helper 鈥� counts by category. Field names (`denied_*`) are kept
 ...
 * success. No grant-flag checkboxes (clipboard/systemKeys) in teach mode 鈥�
 ...
 * must explicitly consent to (main window hides) 鈥� idempotent app grants
...
async function handleRequestTeachAccess(
  ...
  // Same as handleRequestAccess above 鈥� the dialog renders in the hidden
  ...
  // TCC-ungranted branch 鈥� identical to handleRequestAccess's. The renderer
  ...
  if (tccState) {
    ...
    // Same re-check as handleRequestAccess 鈥� user may have granted while the
    // dialog was up, and the pre-dialog snapshot would mislead the model.
    const recheck = await adapter.ensureOsPermissions();
    if (recheck.granted) {
      return errorResult(
        "macOS Accessibility and Screen Recording are now both granted. " +
          "Call request_teach_access again immediately 鈥� the next call will " +
  ...
  // 鈥� skip the dialog entirely. Without this, onTeachPermissionRequest fires
  // with apps:[] and the user sees an empty approval dialog where Allow and
  // Deny produce the same result (granted=[] 鈫� teachModeActive stays false).
  ...
  // skipDialogGrants are pre-existing idempotent app grants 鈥� they don't
  ...
  // app was previously granted (worst case: needDialog=[] 鈫� Allow and
  ...
  return okJson(
    ...
    {
      // response.granted only 鈥� skipDialogGrants are idempotent re-grants.
...
// ---------------------------------------------------------------------------
// teach_step + teach_batch 鈥� shared step primitives
// ---------------------------------------------------------------------------
 ...
 * batch 鈥� same invariant as computer_batch's "coordinates refer to the
 ...
 * prepareForAction ONCE per step (the user clicked Next 鈥� they consented to
 ...
 *
 * Empty `actions` is valid 鈥� "read this, click Next to continue" steps.
...
async function executeTeachStep(
  ...
  // onPermissionRequest 鈥� host stores the resolver, overlay IPC fires it.
  ...
  for (const [i, act] of step.actions.entries()) {
    // Same abort check as handleComputerBatch 鈥� Exit calls stopSession so
    ...
    // Drop mid-step screenshot piggyback 鈥� same invariant as computer_batch.
 ...
 * runs its own prepareForAction 鈥� that's correct: actions may have
 * opened something outside the allowlist. The .screenshot piggyback
 * flows through to serverDef.ts's stash 鈫� lastScreenshot updates 鈫�
...
async function appendTeachScreenshot(
  ...
  if (shotResult.isError) {
    // Hide+screenshot failed (rare 鈥� e.g. SCContentFilter error). Don't
  ...
  return {
    content: [
      ...
      // image]. Spread all 鈥� both notes are useful context and the model
...
async function handleTeachStep(
  ...
  // ok. No screenshot for empty actions 鈥� screen didn't change, model's
 ...
 * steps 鈫� one model鈫扐PI round trip instead of N. Each step still blocks for
 ...
 * user has already clicked through steps 1鈥�4.
 *
 * Anchors for every step scale against the pre-call `lastScreenshot` 鈥� same
 ...
 *
 * Result shape:
 *   {exited: true, stepsCompleted: N}                   鈥� user clicked Exit
 *   {stepsCompleted, stepFailed, executed, failed, 鈥   鈥� action error at step N
 *   {stepsCompleted, results: [...]} + screenshot       鈥� all steps ran
 */
async function handleTeachBatch(
  ...
  // Validate upfront 鈥� fail fast before showing any tooltip.
...
async function buildHiddenNote(
  ...
  return (
    ...
    `${one ? "it" : "one of them"}, that's why you don't see it 鈥� call ` +
...
function uniqueDisplayLabels(
  ...
  // NSScreen.screens iteration order 鈥� same label always maps to same
  // physical display across buildMonitorNote 鈫� switch_display round-trip,
 ...
 * screenshots get no text 鈥� avoids noise.
 */
async function buildMonitorNote(
  ...
  // hot-unplug) must not tank the screenshot 鈥� this note is optional context.
  let displays;
  try {
  ...
  // pre-multimon) 鈥� treat same as undefined.
...
async function handleScreenshot(
  ...
  // 搂2 鈥� empty allowlist 鈫� tool error, no screenshot.
  ...
  // Atomic resolve鈫抪repare鈫抍apture (one Swift call, no scheduler gap).
  // Off 鈫� fall through to separate-calls path below.
  if (subGates.autoTargetDisplay) {
    // Model's explicit switch_display pin overrides everything 鈥� Swift's
    ...
    const result = await adapter.executor.resolvePrepareCapture({
      ...
      autoResolve,
      // Keep the hideBeforeAction sub-gate independently rollable 鈥�
    ...
    // so no retry here 鈥� just a warning when the result is implausibly
    ...
    if (
      ...
      adapter.logger.warn(
        `[computer-use] resolvePrepareCapture result implausibly small (${decodedByteLength(result.base64)} bytes decoded) 鈥� possible transient display state`,
      );
    }
    ...
    // autoResolve (not just appSetChanged) 鈥� when pinned, we didn't
    ...
    // Cherry-pick 鈥� don't spread `result` (would leak resolver fields into lastScreenshot).
  ...
  // 鈥� if a non-allowlisted app is on top, SCContentFilter would composite it
  ...
  if (subGates.hideBeforeAction) {
    ...
    // expected-noise hide. NOT a delta against a cumulative set 鈥� that was
    ...
    // delta to zero. The double-click 鈫� Preview opens 鈫� re-hide 鈫� silent
    // loop never breaks.
    //
    // With this check: every re-hide fires. If the model loops "click 鈫� file
    // opens in Preview 鈫� screenshot 鈫� Preview hidden", it gets told EVERY
    // time. Eventually it'll request_access for Preview (or give up).
    //
    // False positive: user alt-tabs mid-turn 鈫� Safari re-hidden 鈫� reported.
    // Rare, and "Safari appeared" is at worst mild noise 鈥� far better than
...
async function handleZoom(
  ...
  // region: [x0, y0, x1, y1] in IMAGE-PX of lastScreenshot 鈥� same space the
  ...
  if (x1 > last.width || y1 > last.height) {
    return errorResult(
      `region exceeds screenshot bounds (${last.width}脳${last.height})`,
      "bad_args",
    );
  }

  // image-px 鈫� logical-pt. Same ratio as scaleCoord (:198-199) 鈥�
  ...
  // Return the image. NO `.screenshot` piggyback 鈥� this is the invariant.
  return {
...
async function handleClickVariant(
  ...
  // the OS button 鈥� but without this, the JS flag stays true and all
  ...
  // left_click(coordinate=[x,y], text="shift") 鈥� hold modifiers
  ...
  if (args.text !== undefined) {
    ...
    // via native.key(m, "press") 鈥� a non-modifier like "q" in text="cmd+q"
    // gets pressed while Cmd is held 鈫� Cmd+Q fires before the click.
    if (
  ...
  // 搂6 item P 鈥� pixel-validation staleness check. Sub-gated.
  ...
  if (subGates.pixelValidation) {
    ...
    const validation = await validateClickTarget(
      ...
      async () => {
        ...
        try {
          ...
          // selection 鈥� pixel-compare is against the model's last image.
    ...
    if (!validation.valid && validation.warning) {
      // Warning result 鈥� model told to re-screenshot.
...
async function handleType(
  ...
  // 搂6 item 3 鈥� clipboard-paste fast path for multi-line. Sub-gated AND
  ...
  // 搂6 item 7 鈥� grapheme-cluster iteration. Prevents ZWJ emoji 鈫� 锟�.
  // 搂6 item 4 鈥� 8ms between graphemes (125 Hz USB polling). Battle-tested:
  ...
  //      after stripping the newline 鈥� virtualKey 0 is the 'a' key, so a
...
async function handleKey(
  ...
  // 搂2 鈥� blocklist check BEFORE gates. A blocked combo with an ungranted
  // app frontmost should return the blocklist error, not the frontmost
  // error 鈥� the model's fix is to request the flag, not change focus.
  if (
...
async function handleScroll(
  ...
  // up 鈫� dy = -amount; down 鈫� dy = +amount; left 鈫� dx = -amount; right 鈫� dx = +amount.
  ...
  // a leftMouseDragged event (enigo reads NSEvent.pressedMouseButtons) 鈥�
...
async function handleDrag(
  ...
  // across the drag and desyncs the flag from OS state 鈥� same mechanism as
  ...
  // (required). `start_coordinate` is OPTIONAL 鈥� when omitted, drag from
  ...
  // else: rawFrom stays undefined 鈫� executor drags from current cursor.
  ...
  // omitted the drag begins at the cursor 鈥� same bypass as mouse_move 鈫�
  ...
  // right-click鈫扨aste. `from` stays "mouse" 鈥� picking up is a read.
...
async function handleMoveMouse(
  ...
  // the window under the cursor 鈥� that's interaction, not positioning.
  ...
  // hit-test the cursor to close the mouse_move鈫抣eft_mouse_down decomposition).
  ...
  if (mouseButtonHeld) {
    // "mouse_full" 鈥� same as left_click_drag's to-endpoint. Dragging onto a
...
async function handleOpenApplication(
  ...
  // Resolve display-name 鈫� bundle ID. Same logic as request_access.
  ...
  if (looksLikeBundleId(app) && allowed.has(app)) {
    ...
    // Try display name 鈫� bundle ID, but ONLY against the allowlist itself.
  ...
  // open_application works at any tier 鈥� bringing an app forward is exactly
  ...
  // clicking on dock icons. Single-monitor 鈫� no hint. listDisplays failure is
  // non-fatal 鈥� the hint is advisory.
    ...
    if (displayCount >= 2) {
      return okText(
        `Opened "${app}". If it isn't visible in the next screenshot, it may ` +
          `have opened on a different monitor 鈥� use switch_display to check.`,
      );
    }
  }
...
async function handleSwitchDisplay(
  ...
  // Resolve label 鈫� displayId fresh. Same source buildMonitorNote reads,
...
async function handleReadClipboard(
  ...
  // read_clipboard doesn't route through runInputActionGates 鈥� sync here so
  ...
  // clipboardGuard may have stashed+cleared 鈥� read the actual (possibly
...
async function handleWriteClipboard(
  ...
  if (subGates.clipboardGuard) {
    ...
    if (frontmost && frontmostTier === "click") {
      return errorResult(
        `"${frontmost.displayName}" is a tier-"click" app and currently ` +
          ...
          `would clear the clipboard anyway 鈥� a UI Paste button in this ` +
    ...
    // write_clipboard doesn't route through runInputActionGates 鈥� sync here
 ...
 * No frontmost gate 鈥� no input, nothing to protect. Kill-switch + TCC
 ...
 * coordinateSpace annotation 鈥� the model handles both shapes.
 *
 * When lastScreenshot is present: inverse of scaleCoord 鈥� logical points 鈫�
 * image-pixels via `imageX = logicalX 脳 (screenshotWidth / displayWidth)`.
 * Uses capture-time dims so the returned coords match what the model would
 * read off that screenshot.
 *
 * No frontmost gate 鈥� read-only, no input.
 */
 ...
 * as wait. Keyboard action 鈫� frontmost gate applies; same systemKeyCombos
...
async function handleHoldKey(
  ...
  // Blocklist check BEFORE gates 鈥� same reasoning as handleKey. Holding
...
async function handleLeftMouseDown(
  ...
  // overlapping a tier-"full" frontmost app 鈥� bypassing runHitTestGate's
...
async function handleLeftMouseUp(
  ...
  // Any gate rejection here must release the button FIRST 鈥� otherwise the
  ...
  // vector) 鈥� hit-test at "mouse_full" same as left_click_drag's `to`. When
  // NO move happened, this is a click-release 鈥� same semantics as the atomic
...
/**
 * Executes `actions: [{action, 鈥, 鈥`
 * sequentially in ONE model鈫扐PI round trip 鈥� the dominant latency cost
 ...
 *   - Frontmost gate: checked PER ACTION. State can change mid-batch 鈥� a
 ...
 * false, pixelValidation: false}` to each inner dispatch 鈥� the handlers'
 ...
 *
 * Mid-batch screenshots are allowed (for inspection) but NEVER piggyback 鈥�
...
async function handleComputerBatch(
  ...
  // design). Frontmost still checked 鈥� runInputActionGates does it
  // unconditionally.
  ...
  const results: BatchActionResult[] = [];
  for (const [i, act] of actions.entries()) {
    // Overlay Stop 鈫� host's stopSession 鈫� lifecycleState leaves "running"
    // synchronously before query.interrupt(). The SDK abort tears down the
    // host's await but not this loop 鈥� without this check the remaining
    ...
    if (inner.isError) {
      ...
      // reflects the actual failure 鈥� without this, batch-internal errors
 ...
 * AFTER kill-switch + TCC gates have passed. Never sees request_access 鈥� it's
...
export async function handleToolCall(
  ...
  //     added the app to Settings 鈫� Desktop app 鈫� Computer Use 鈫� Denied apps)
  //     must not survive. Without
  //     this, a stale grant bypasses the auto-deny. Stripped silently 鈥� the
  ...
  //     `tier: undefined`, which `tierSatisfies` treats as `"full"` 鈥� wrong
  ...
  // 鈹€鈹€鈹€ Gate 1: kill switch 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  ...
  // 鈹€鈹€鈹€ Gate 2: TCC 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  // Accessibility + Screen Recording on macOS. Pure check 鈥� no dialog,
  ...
  // 鈹€鈹€鈹€ Gate 3: global CU lock 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  // At most one session uses CU at a time. Every tool including
  // request_access hits the CHECK 鈥� even showing the approval dialog while
  ...
  //
  // request_teach_access is NOT in this set 鈥� approving teach mode HIDES
  ...
  // singleton. When undefined (tests/future hosts), no gate 鈥� absence of
  // the mechanism 鈮� locked out.
  ...
  if (lock) {
    ...
    if (lock.holder === undefined && !deferAcquire) {
      // Acquire. Emits cuLockChanged 鈫� overlay shows. Idempotent 鈥� if
      ...
      // Fresh lock holder 鈫� any prior session's mouseButtonHeld is stale
    ...
    // lock.isSelf 鈫� already held by us, proceed.
    // lock.holder === undefined && deferAcquire 鈫�
    //   checked but not acquired 鈥� proceed, first action will acquire.
  }
  ...
  // handleReadClipboard/handleWriteClipboard. NOT here 鈥� per-tool-call sync
  ...
  // 鈹€鈹€鈹€ Fail-closed dispatch 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  // ANY exception below 鈫� tool error, executor never left in a half-called
  // state. Explicit inversion of the prior `catch 鈫� return true` fail-open.
  try {
    ...
    // the gated tools 鈥� the gates run before the executor in every handler.
    // For ungated tools, the executor may have been mid-call; that's fine 鈥�
