...
> 运行时口径更新（2026-04-15）：当前默认 runtime backend 已切到 `pi-mono`。本文中保留的 `OpenCode` / `opencode-adapter` 叙述，主要用于解释历史镜像写链、上游运行时演进和兼容收口背景，不应再解读为当前默认 runtime 接入实现。
>
> 状态：Draft v4
> 日期：2026-03-31
> 作者：GitHub Copilot
> 关联文档：[opencode-runtime-protocol.md](../archive/runtime/historical-opencode-runtime-protocol.md)、[runtime-process-architecture.md](../architecture/runtime-process-architecture.md)、[task-session-message-roundtrip-target-plan.md](task-session-message-roundtrip-target-plan.md)
>
...
1. **当前镜像写链**（§ 2 - § 9）：task session message 如何从当前 runtime backend（默认 `pi-mono`）写入 control-plane 数据库。描述的是当前代码已经在执行的实际链路。
2. **历史上游执行链**（§ 10 - § 11）：旧 OpenCode runtime 内部从 HTTP 路由到 LLM 流式输出再到全局 SSE 的完整链路，主要保留为历史实现参考。
3. **流式差距与改造方向**（§ 12 - § 15）：当前外层为什么只做到"较粗粒度增量"，以及历史 OpenCode 底层如何持久化 session/message/part 和模型数据。
4. **消息写入策略详述**（§ 17 - § 21）：单执行、并行执行、顺序链三种模式下的写入策略，特别是并行候选的 user prompt 路由/去重、event log 与 projection 的分层写入、winner 选择后的可见性切换。

一句话结论：

**当前消息正文的第一事实源是当前 runtime backend（默认 `pi-mono`）；BFF 通过 `runtime-provider` 的实时事件与消息读取接口观察消息变化，在必要时回读完整 message snapshot，再把该 snapshot 发送给 control-plane service；service 最终把它写入 `task_sessions`、`task_session_runs`、`task_messages`、`task_message_parts` 和 `task_timeline_views`。**
...
sequenceDiagram
  ...
  participant Adapter as BFF runtime-provider
  participant RT as Runtime Backend (pi-mono)
...
1. [control-plane/web-ui-bff/src/modules/agent-control/runtime-provider.ts](../../control-plane/web-ui-bff/src/modules/agent-control/runtime-provider.ts) 的 `createSession(...)`
2. 同文件里的 `continueSession(...)`
3. 同文件里的 `getSessionMessages(...)`
4. 默认实现由 [control-plane/web-ui-bff/src/modules/agent-control/runtime-provider-pimono.ts](../../control-plane/web-ui-bff/src/modules/agent-control/runtime-provider-pimono.ts) 提供

这一层当前实际调用的是 runtime provider 抽象，而不是旧 OpenCode HTTP 接口：

1. `createSession(...)`
2. `continueSession(...)`
3. `getSessionMessages(...)`
...
runtime 的实时变化进入 BFF 后，会被统一送进实时聚合器：

1. [control-plane/web-ui-bff/src/modules/realtime/sse-aggregator.ts](../../control-plane/web-ui-bff/src/modules/realtime/sse-aggregator.ts) 的 `transformEvent(...)` / `processParsedEvent(...)` 把 runtime 原始事件转换为内部 `RealtimeEvent`
2. 同文件里的 `resolvePersistableMessageSnapshot(...)` 负责把不完整事件补齐成完整 message snapshot
...
下面追加三大节，将视角向上延伸到 **历史 OpenCode runtime 本体**：
...
## Part 2 — 历史 OpenCode Runtime 执行链路
