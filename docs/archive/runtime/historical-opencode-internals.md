# Historical: OpenCode 底层运行原理与 Git 设计

> 文档类型：历史资料
> 历史注记（2026-04-15）：当前默认 runtime 已切到 `pi-mono`。本文保留的是 OpenCode 时代的内部机制说明，主要用于回溯旧实现、兼容语义和历史设计判断。
...
详细协议规范见 [opencode-runtime-protocol.md](historical-opencode-runtime-protocol.md)。
