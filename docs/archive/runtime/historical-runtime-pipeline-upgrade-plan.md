...
> 归档说明：本文档现按历史方案归档。它保留的是旧 runtime pipeline 升级设想与当时的实现语境，不再作为当前实现说明，也不再作为现行目标方案入口。
> 状态说明：本文档中涉及旧版图模型兼容层的设计已经过时。
> runtime 状态说明：本文中把执行主链写成 `OpenCode Runtime + SSE Aggregator` 的段落，保留的是当时的实现语境。当前默认 backend 已切到 `pi-mono` runtime-provider；这里讨论的 pipeline 仍应理解为对 runtime backend 与事件聚合层的视图投影，而不是今天默认主链仍直接以 OpenCode 为中心。
> 当前实现已移除独立 DAG 兼容层；并且 runtime pipeline 主路径已经不再读取 `tasks.executionPlan` / `parallelRunHistory`。并行任务以 task domain runs 为主数据源，规划阶段仅作为补充展示层。阅读本文件时，请以 [docs/dag-node-execution-plan-v2.md](../../workflow/dag-node-execution-plan-v2.md) 的清理结论为准。
...
它不是独立的 DAG 执行引擎（执行仍由 runtime backend 与事件聚合层承担；旧文中的 `OpenCode Runtime + SSE Aggregator` 只是历史实现语境），而是一个**聚合视图层**，从多个已有数据源计算出统一的流水线状态。
