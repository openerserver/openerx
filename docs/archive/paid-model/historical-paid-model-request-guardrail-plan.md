...
其中并行 candidate 的成本随候选数近似线性增长，[../../multi-agent-hook-architecture.md](../../multi-agent-hook-architecture.md) 已明确指出这一风险，但当前缺少强制执行的上限机制。
...
但根据 [../../architecture-overview.md](../../architecture-overview.md) 的判断，这些能力尚未与运行时内核形成强约束闭环。
...
1. runtime usage ledger 三层账本尚未建成。文中建议的 `runtime_usage_ledgers`、`runtime_usage_ledger_steps`、`runtime_usage_baselines` 目前还没有作为正式表结构进入控制面，因此当前还不能在项目治理页完整回答“底层到底打了几次模型、每一步谁放大了请求”。
