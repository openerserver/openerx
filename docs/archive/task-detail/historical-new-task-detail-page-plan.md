...
control-plane/web-ui/src/
  ...
  composables/
    ...
    useTaskMessageSnapshot.ts               # round snapshot 读取、refresh 编排
    useTaskMessageStore.ts                  # 主聊天 reducer、streaming 状态
...
| useTaskMessageSnapshot.ts / useTaskMessageStore.ts | 220-320 | round snapshot 读取 + unified message reducer / streaming |
...
1. 新建 `useTaskMessageSnapshot.ts` + `useTaskMessageStore.ts`，负责消息拉取、streaming 状态与统一 reducer。
...
3. `useTaskMessageSnapshot.ts` / `useTaskMessageStore.ts`：消息加载、streaming 判断。
