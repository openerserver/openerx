...
>
> 其中关于 `task-graph-plugin`、graph API、`task_nodes` / `task_edges` 数据模型的差距描述已被后续清理工作覆盖。当前实现状态请以 [dag-node-execution-plan-v2.md](../workflow/dag-node-execution-plan-v2.md) 和现行 schema / migration 为准。
>
...
- [OpenCode 关注边界说明](../archive/runtime/historical-opencode-focus-boundary.md)
- [OpenerX 当前系统架构说明](../architecture/architecture-overview.md)
- [OpenerX 目标架构演进图](../architecture/architecture-target-evolution.md)
