...
> runtime 状态说明：本文成稿时默认候选执行链仍围绕旧 OpenCode runtime。当前默认 backend 已切到 `pi-mono` runtime-provider；文中涉及 `opencode runtime` 的地方，应理解为历史执行拓扑或 runtime backend 占位，而不是今天的默认主链命名。
...
```text
┌────────────────────┐       ┌─────────────────────┐       ┌────────────────────────────┐
│  ExecutionModeModal │──────►│  BFF /tasks/:id      │──────►│  runtime backend            │
│  (配置并行候选)      │       │  /execute            │       │  (default pi-mono; legacy   │
└────────────────────┘       │  /candidates/:i/adopt│       │   OpenCode adapter path)    │
           └─────────────────────┘       └────────────────────────────┘
                                      │
                              SSE Aggregator
