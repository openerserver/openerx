...
> 文档类型：当前实现
> 日期：2026-04-05
> 目标：在保留 OpenerX 控制面与任务域语义的前提下，用 pi-mono 接管旧 OpenCode runtime 执行内核。
...
10. 更早一轮真实验收的边界曾经只覆盖“execute 路由成功启动真实 pi-mono session，并完成 task identity fallback patch”；该限制已在后续 live roundtrip 验收中消除，当前 assistant 最终回复、tool result 回流和 trace 读面都已进入真实验收范围。
...
| OpenCode provider 封装 | 是 | 是 | 保留为回退兼容路径 |
...
| runtime permission bridge | 是 | 是 | provider 专项测试通过；该项只描述上游 `extension_ui_request` 桥接，结构化 `external_directory` / `command_execution` 审批由 Phase 5 的 governance extension 补齐 |
...
2. 后续如果要继续推进兼容层收口与长期稳态，建议再补一份更细的实施文档，至少拆出 provider 内部模块拆分、RPC 协议映射表、事件到 task-domain 写模型的映射规则，以及兼容层退场与回滚方案。
