...
| M6 Remote Control Alpha | LOCAL COMPLETE | Expo 手机控制面、同账户配对、E2EE 协议、出站 Connector、密文 Gateway、Pi 原生命令映射、远程审批和事件游标；2026-09-15 补齐附件上传与账户完整历史 | iOS/Android 真机、Windows/macOS 主机矩阵、APNs/FCM 生产投递、附件双端真机上传/打开与真实网络故障演练仍是发布门禁；[M6 证据](evidence/m6-2026-08-26.md)；[附件与历史证据](evidence/mobile-attachments-history-2026-09-15.md) |
...
2026-09-15 补齐手机附件上传、桌面校验/解析/消息绑定，以及账户全量分页历史、归档和分支阅读、加密离线缓存。新增 HTTP 集成测试与 Electron E2E 覆盖附件进入执行链路；iOS 模拟器确认配对前的 55 项历史可读取并在重启后恢复。详见 [附件与历史检查点](evidence/mobile-attachments-history-2026-09-15.md)。

本状态不等于上述 Remote Alpha 发布退出条件已经通过。iOS/Android 真机与 Windows x64、
macOS arm64/x64 组合，生产 APNs/FCM、真实 HTTPS/WSS 部署、附件双端真机上传/打开以及休眠、
