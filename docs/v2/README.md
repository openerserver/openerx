...
> M3、M4 与 M5 本地实现检查点已完成，当前新主线只在 `apps/`、`services/` 和 `packages/` 推进。
...
3. 实现按 [13-development-plan.md](13-development-plan.md) 的阶段退出条件推进；M5 本地实现
   检查点已完成，当前进入 M6，不设置旧执行引擎迁移阶段。真实模型、支付沙箱、属地合规和
   原生双平台证据仍按发布 Gate 补齐。
