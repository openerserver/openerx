...
| TOOL-08 | 工具过程 | 工具调用在对话中可折叠查看，包含状态、输入/目标摘要、结果、来源、耗时和错误；原始协议不作为普通用户主界面 |
| TOOL-09 | 取消、重试与幂等 | 可停止正在运行的工具；外部写操作使用幂等键，重试不得重复产生副作用 |
| TOOL-10 | 沙箱与审批 | 默认只写授权工作区且 Shell/脚本无任意网络；网络、浏览器、桌面控制和外部写入按风险分级，越出 Scope 前必须请求授权 |
...
| SKILL-06 | 脚本和资源 | Skill 可以包含脚本、参考资料和模板；脚本只能通过同一 Tool Gateway 和权限系统执行 |
