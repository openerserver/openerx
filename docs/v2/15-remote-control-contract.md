...
- 手机提供“本次允许”“本会话允许”“完全允许”和“拒绝”；不创建设备级“始终允许”。“本会话允许”仅用于 L0–L3，按当前会话、资源、动作和原有 Scope 生命周期复用，包含后续远程 Bash 调用。
- “完全允许”通过 `permission.decide / full_access` 启用当前会话的完全访问，同时释放该会话已等待的审批；不会扩大工作区或操作系统授权，可在桌面输入框恢复请求审批。
- 审批显示精确目标、动作、风险和有效期。手机前台点击确认后直接提交，不触发 Face ID／生物识别；主机继续校验原控制器、会话、请求状态、payload digest、设备解锁和 60 秒内的决定时间。为保持线协议兼容，决定时间沿用 `reauthenticatedAt` 字段，`biometricVerified` 为 false。
- L4/L5 不提供“本会话允许”；用户可选择“本次允许”或明确启用当前会话的“完全允许”。
...
- 该检查点仅支持文字任务，附件入口暂时移除，最近任务来自手机保留的加密事件。附件上传与账户完整历史在下述 2026-09-15 检查点补齐。
...
### 9.2 2026-09-15 附件上传与账户历史

- 手机可选择文件、照片或拍照；每次最多 10 个附件、合计 50 MB，复用桌面支持的文件格式。附件随新任务或下一轮提问发送；当前回复结束后才可添加附件，Steer/Queue 仍提交文字。
- 附件字节通过账户对象服务上传；加密远程命令只携带对象引用、文件名、大小、格式及 SHA-256。桌面按当前账户下载并校验，复用 File Service 解析，将附件绑定到准确的用户消息，再调用 Pi。主机通过 `attachment.upload` 声明支持。
- 上传失败保留当前任务草稿与文件；同一次草稿重试复用已成功上传的对象。错误引用、跨账户对象、大小或摘要不符在创建任务和调用模型前拒绝。
- “历史任务”从账户 Sync API 分页获取完整已同步的会话、分支、消息及附件记录，支持搜索、已归档任务和历史分支阅读。实时 Remote 事件仅补充尚未同步的进度，旧事件不能覆盖完整回答或恢复已删除记录。
- 历史按账户加密缓存到手机文件，密钥存入 SecureStore；逐页落盘后推进游标，重启、临时离线或主机未连接时仍可阅读已保存内容。退出或设备会话被撤销后清理缓存。桌面尚未上传的本地数据仍需先完成账户同步。

验证命令、原生模拟器观察和剩余真机边界见 [附件与历史验证记录](evidence/mobile-attachments-history-2026-09-15.md)。

### 9.3 2026-09-15 连接状态与恢复修复

- `degraded` 显示为“连接不稳定，正在自动重试”，并与 Gateway 现有可达性规则保持一致；实际离线、关闭 Remote 或撤销时仍禁止发送。
- 任务是否可编辑与电脑是否连接分别判断。查看归档任务或历史分支显示对应的只读状态，不再误报“电脑未连接”。
- 手机发现、事件拉取和登录续期，以及桌面 Remote HTTP 请求增加超时；手机回到前台立即检查主机与事件。桌面初始连接失败可重试，状态写入响应丢失后重新读取服务端 revision 并恢复。
- 状态恢复保持 Remote 的关闭/撤销决定；新命令读取会话状态失败按任务错误返回，不把它误判为网络掉线。

代码路径、故障注入和 iOS 原生显示验证见 [连接恢复验证记录](evidence/mobile-connection-recovery-2026-09-15.md)。
