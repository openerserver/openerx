...
> 历史检查点：本文记录的是自动抽取尚未接入产品运行时之前的安全基础。后续状态已由
> [Memory Phase B 运行时与多来源检查点](memory-phase-b-runtime-sources-2026-08-30.md) 取代；其中已按
> 服务端权威计费架构移除客户端 remaining-percent 门禁，并开放自动生成、通知和撤销。
