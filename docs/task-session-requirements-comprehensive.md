...
- **OpenCode Runtime（运行时）**：实际代码执行、LLM 调用、工具执行
- **BFF（前端聚合层）**：统一入口、认证代理、runtime 适配、任务聚合

> **关键分离**：Task/Session 是 Control Plane 的持久化概念；runtime session 是 OpenCode Runtime 的执行概念。两者通过 `runtimeSessionId` 关联，但 ID 形态不同。
...
- **Runtime Session ID**: OpenCode 运行时原始 session ID（如 `ses_xxx`）
...
**不是源**：OpenCode runtime `GET /session/:id/message` — 底层协议/诊断接口，不是公开 trace fallback
...
- 不 fallback 到 opencode runtime messages
