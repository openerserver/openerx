...
当前 service 也已经按这个边界在写：消息正文写入 `conversation_messages` / `conversation_message_parts`，而不是写入 session 表本身。[task-conversation-message-sync.ts](control-plane/service/src/modules/tasks/task-conversation-message-sync.ts#L405)
